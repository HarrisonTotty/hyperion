...
#[test]
#[ignore = "TODO: rewrite against current API response shapes (ListPlayersResponse, snake_case roles, etc.)"]
...
#[test]
#[ignore = "TODO: rewrite against current API response shapes and role casing"]
...
#[test]
#[ignore = "TODO: rewrite against current API response shapes"]
...
fn test_power_and_cooling_allocation() {
    ...
    let response = client.get("/v1/ships/test-ship/status").dispatch();
...
fn test_countermeasure_vs_missile_combat() {
    ...
    let result = calculator
        .calculate_damage(base_damage, &antimissile_tags)
        .unwrap();
    ...
    let result = calculator
        .calculate_damage(base_damage, &chaff_tags)
        .unwrap();
...
fn test_combat_scenario_with_weapon_tags() {
    ...
    let result = calculator
        .calculate_damage(base_damage, &plasma_tags)
        .unwrap();
    ...
    assert_eq!(
        result.status_effect.unwrap().effect_type,
        StatusEffectType::IonJam
    );
