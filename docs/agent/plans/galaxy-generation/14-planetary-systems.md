...
- **P14.T41–T43 for bodies, and ruling 59's fixes, as built (round 7d, `ui`).** Against
  ...
  - **For the orchestrator to rule:** (1) a dwarf planet at the floor is a smaller planet's size,
    ...
    before and after).
- **Rulings 58, 61.3 and 62.5 at the merge (`integ`, round 7).** No pinned value moved by any of the three; `planetary/derive_body` and `planetary/context` gain labels that pin the corrected branches, blessed at 11.
  - _Ruling 58._ `RadiusWindow` gains `dry_top()` and `dry_top_core_mass_fraction()`: the rock curve and 0 inside the snow line, the Earth-like curve and 0.325 beyond it. `rocky::core_mass_fraction_at_least(rank, floor)` is Plotnykov and Valencia's distribution conditioned on a core fraction of at least `floor`, and `rocky_core_mass_fraction` maps the rocky part of the rank onto it, so inside the snow line nothing changes, bit for bit. Of 8,991 bodies of 0.1–1.9 M⊕ beyond the snow line (999 ranks, nine masses, 0.05 F⊕), 5,071 are watery again (2,596 under ruling 53 alone), and every dry one has a core fraction of 0.325 or more. `planetary/derive_body`'s bodies beyond the snow line all lie above the Earth-like curve, so none moved; it gains `beyond_dry` and `beyond_watery`, 1 M⊕ at 4 au with their ranks halfway along the rocky part and between the Earth-like and rock curves.
  - _Ruling 61.3._ `SystemContext::zone_stars` gives a star below 0.1 M☉ (`sse::MIN_INITIAL_MASS`, where `StarModel` leaves its track for the cooling fits) the fits' state at 5 Gyr (`COOLING_MAIN_SEQUENCE_AGE`): 2.5 × 10⁻⁴ L☉ at 0.08 M☉ and solar composition, within 1% of Tout et al.'s at 0.1 M☉ at [Fe/H] −1, 0 and +0.3 (tested). **For the orchestrator:** the 5 Gyr is the `context` lane's measure of "the fits' main sequence"; a metal-poor star of 0.08–0.083 M☉ lies below the fits' hydrogen-burning limit and never settles, so its 5 Gyr state is a cooling one. None of `planetary/context`'s hosts is below 0.1 M☉, so none moved; it gains a 0.09 M☉ host at \[Fe/H\] −0.3.
  - _Ruling 62.5._ Each lane had its own copy of the damping law, e₀ exp(−t ÷ τ) at constant a(1 − e²). `hosts::evolved`'s now calls `tides::circularise`, which gives the same bits (the `planetary/fate` golden holds), and the generator computes each τ_c with `tides::circularisation_time`. No `place`-owned file was edited.
- **Deviations in T30, as built (`integ`, round 7).**
  - _T30.a, shape._ `system::{generate, generate_planets}(Seed, &SystemContext) -> PlanetarySystem`, re-exported from `planetary`, and `generate` is `generate_planets` in the slice. `PlanetarySystem` has `system`, `zones() -> &[OrbitZone]`, `hosts()`, `host(OrbitHost)`, `zone(OrbitHost)`, `disc(OrbitHost) -> Option<&Disc>`, `architecture(OrbitHost) -> Option<ArchitectureClass>` (`None` for a host that is none of its zones, where Provides has no `Option`), `bodies()` sorted by index, `body(BodyIndex)` and `children(BodyIndex)` (empty in the slice). `PlanetaryHost` holds the host, its limits, disc, plane (`SystemPlane`), drawn class and placed class. `Body` holds the `PlacedPlanet`, its `OrbitHost`, its radius rank and its `FateBody`. `belts()`, `halo()` and `events_between` wait for T21 and T31, whose types do not exist.
  - _T30.a, the chain._ Per zone of `ctx.zones()`: `ZoneDiscInputs::for_zone` for the disc's host, lifetime and draws; `disc::derive` cut to the zone's truncation and to the context's strip radius where the zone reaches further or has no outer limit; `draw_class` under `ClassConstraints::new(&disc, reach, zone.host_multiplicity())`; `place` from slot 1, each host from the last one's `next_slot`, in the host's plane (a pair under 47 au lends its orbital plane to its circumbinary zone, the rule of `tests/planetary_support`), else drawn. A host reached after slot 191 keeps its disc and class and places nothing, rather than spill into slot `0xC0`. **For the orchestrator:** the strip-radius cut is T29's entry point, taken now because P14.T1.d already gives the radius and D14 asks for nothing beyond it; it changes no bit wherever the strip radius lies beyond the disc's edge, which the chain test checks on 150 real systems against the stages called directly.
  - _T30.a, each planet's draws._ `planet.radius` (`Body`) is registered after `planet.origin` (`tags.golden` gains that line): word 0 is the rank, words 1–7 reserved (`RADIUS_WORDS`), `system::radius_rank(seed, BodyId)`. `Formation::draw` takes the zone's disc lifetime. τ_c is `circularisation_time` at the primordial orbit about the host's initial mass, with Q′ of 10⁶ from 0.1 M_J (`SPACING_GIANT_MASS`) and 10² below, and the `FateBody`'s density is the mass over the same radius's sphere. **For the orchestrator:** that radius is Chen and Kipping's median at the mass, the radius the placer's Roche limit already takes and T8.e's tests used, since the derived radius depends on the time and the fate transform needs one fixed at birth.
  - _T30.b, API._ `body_at(&ctx, index, t) -> Result<BodyRecord, ResolveBodyError>`, `snapshot_at(&ctx, t) -> SystemSnapshot` (records in index order, belts and halo `NotModelled`), `position_at(&ctx, index, t) -> Result<Option<SystemPosition>, ResolveBodyError>` (`None` for a body not present: an addition to Provides' shape, since such a body has no position and is no error) and `habitable_zone_at(&ctx, host, t) -> Option<HabitableZone>` (`None` for a host that is no zone, or before its stars form). A star's index is `NoSuchBody`: stars' records are plan 06's. `t` is a time of the clock window, as for `StarModel::state_at`; T36 validates it. A context other than the one the system was generated from is a caller's bug, and every query panics on it (the IDs are compared).
  - _T30.b, records._ The identity is a `Planet`, labelled, with its zone's host as parent and the fate transform's state; the mass is always `Ok`. A present body has its orbit at `t` (`FateAt::body_orbit`, with `valid_until`), its position and `derive_body`'s bulk and surface tags. **For the orchestrator:** a body not yet formed, destroyed or unbound has its orbit, bulk and surface `NotApplicable`, since nothing orbits there at `t`, and its mass as it was. Moons, rings and hooks are `NotModelled`. The derivation reads the zone's stars' `StarState`s at `t` as `HostLight`s, every other star as an `Illumination` along the orbit of the lowest pair that holds both it and the host, and `BodyFate::host_mass(t)` (made public) as the primary.
  - _T30.b, positions._ The host's position follows `star_positions_at`'s walk with its arithmetic, bit for bit for a star (tested at ±H and the epoch on every sampled system) and a pair's barycentre on the same walk (its members' mass-weighted mean to 10⁻¹²); the body adds its fate orbit's `relative_state_at(t)`. The host's reflex about its planets is not modelled.
...
- **Deviations in T36, as built (`srvbodies`, round 7),** the slice: `system_bodies` and `body_detail`; `body_events` still answers `unsupported` until T31.
  - _T36.a, the cache._ `compute/bodies.rs`: `SharedBodyCache`, a `SharedByteLru<(GalaxyKey, SystemId), (SystemContext, PlanetarySystem)>` (the tuple is `GeneratedSystem`), filled under a `SingleFlight` whose one interactive pool job builds the context and runs `generate` in the universe of the key's seed; `BodyCacheCounters { cache, generated }` counts generations, so the "generated once" tests read a counter. The miss path takes the stars from the `SystemStars` cache (P06.T34) and builds the context with a new `SystemContext::from_stars(galaxy, &SystemStars)`, of which `from_record` is now a call, bit for bit (tested), so the stars are evolved once for both caches. `HeapBytes` is the sim's new `SystemContext::heap_bytes` (the copied models and tracks, as `SystemStars::heap_bytes` counts them) plus `PlanetarySystem::heap_bytes` (zones, hosts and bodies by capacity; none owns more). Measured over 701 systems of seed `0x4d2` at the solar circle (2,968 bodies), an entry is charged 3.7 KB (no planets) to 71.6 KB, median 8.8 KB and mean 13.6 KB, most of it the stars' tracks. **For the orchestrator to rule: the budget is its own,** `--body-cache` (`HYPERION_BODY_CACHE_MB`, default 128 MiB, about 9,900 systems), not shared with the stars cache, so that a sweep of the `GALAXY` display's contacts cannot evict the systems a `SYSTEM` display has open, and because one `SharedByteLru` holds one kind of value; the README's table and `config.rs` gain the row, and `ServerStats::bodies()` the counters.
