...
- **Deviations in T11, as built.** _T11.a:_ `toScene(result, { frame, driveRangeLy, selectedId, destinationId })` is as sketched; `chartModel.ts` also holds `DEFAULT_DRIVE_RANGE_LY` (50 ly, D9), `distanceDecimalsFor(queryRadiusLy)` (D21's one precision per chart), `queryRadiusForDriveRange(driveRangeLy)` (`ceil125` clamped to the steps, which is where T11.c's "an untouched radius follows the drive range" is decided, so that the component stays presentational), `inRangeCount(result, driveRangeLy)`, and two length formats: `formatChartLengthLy` and `formatBandMsun` write a 1-2-5 radius, an entered range and a band edge with as few decimals as the value needs (`50`, `0.05`, `37.5`, `0.08`), since these are settings and band boundaries, not measurements, and the plan's own labels (`RANGE 50 ly SET`) are written that way. `censusLine` and `censusHint` return the annunciated condition in capitals and the action in mixed case, as T6 and T8.g have every other message: `NOTHING FITS: reduce radius`, `DENSE REGION: reduce radius before raising MIN MASS`, `LARGE VOLUME: layers dropped by the server cell budget; reduce radius` (the guide asks an error to say what the operator can do, which the plan's second hint did not). `layerBands` throws when a census does not list all five layers. _T11.b:_ `useRangeQuery` takes one object, `RangeQueryInput`, with a sixth member, `generation`, which `RETRY` bumps, since `useServerRequest` needs it to send a settled body again; the body is compared by value by the hook below it, so it is built afresh each render and not memoised. _T11.c:_ `ChartControls` is presentational: it takes `radiusChoiceLy` and `queryRadiusLy` and reports choices. The mass floors' unit is drawn once on the group's legend (`MIN MASS M☉`), which the guide's "a unit once for a labelled group" allows, since five options each carrying `M☉` do not fit the column; the options read `ALL 0.08`, `0.5`, `0.75`, `2.5`, `8`, and the layer letters until the first census. `DRIVE RANGE SET` and `CHART TIME` are entered as their field is left or with `Enter`, as T8.g's coordinates are, with the hint beside the unit (`0.01-500 ly`, `±1000 yr`) and the refusal in T8.g's words (`DRIVE RANGE INVALID: enter 0.01 to 500 ly`, `CHART TIME INVALID: enter -1000 to 1000 yr`), not the plan's `OUT OF RANGE: …`. Every control is held back with `aria-disabled` and described by the link's reason rather than disabled, as `OPEN` and `CREATE` are. _T11.d:_ `CensusReadout` puts the census line, the request's state, `SYSTEMS`, `IN RANGE` and the control that shows the census by layer on one line; the per-layer table folds (see the ruling below). _T11.e:_ `SystemList` holds its own scroll offset, sets `scrollTop` from it and reads the element only through a ref, so that a scroll by hand never snaps the selection back and a programmatic scroll needs no scroll event; its columns' units are named once in a head row (`DIST ly`, `INIT MASS M☉`), and each row's accessible name carries them (`H7K 4C0RFZ A-1, 1.00 ly, 0.29 solar masses, IN RANGE`). With nothing selected the nearest system is the active row, so the plan's "`{ArrowDown}` three times selects the fourth-nearest" holds. One click handler serves the whole list, from `data-index` on the row, so that a row carries no listener of its own; `role="listbox"` and `role="option"` need an oxlint suppression each. _T11.f:_ `SystemReadout` takes the frame, the range and the chart time as props and derives `HEIGHT`, `COREWARD` and `SPINWARD` from the offset with `toLocal`; `ANGLE` carries its degree sign against its digits, as the cursor readout's does. _T11.g:_ the chart's state (the radius, the floor, the range, the time and the selection) and its query live in `displays/galaxy/useLocalChart.ts` (`useLocalChart(centreLy)`, `LocalChartState`), called by `GalaxyDisplay`, because the chart's picture and the list and readout beside it now stand in different columns (see the ruling below); `LocalChartPanel` takes that state as one prop and formats the view's furniture from it. The selection is derived from the answer on show rather than cleared, which is the plan's "keeps the selection if the system is still present and clears it otherwise". Added files beyond the plan's list: `useLocalChart.ts` and `SystemsPanel.tsx`.
- **Orchestrator's rulings for the owner, applied with T11.** (1) _The chart is the third page of the shared panel, and its list and readout take the second column._ Measured in headless Chromium with the bundled B612 (the display's DOM from jsdom under the real stylesheet; harness outside the tree), the second column cannot hold the chart at 1280 × 688: the work area is 604 px, `.galaxy` 572 px, and with `CURSOR` at 166 px the chart's panel gets 390 px, 372 px inside, while the view's own controls row (76 px), stage floor (12 rem) and furniture row (84 px) need 360 px in a 22.9 rem column before any control, census, list or readout. So `GalaxyPages` gains a third page, `LOCAL CHART`, beside `PARAMETERS` and `GALAXY MAP` — which is where T6.d's sketch put the chart's view, in the display's wide column — and the second column holds `CURSOR` above the new `SYSTEMS` panel, the list and readout of T6.d's fourth column. `CENTRE CHART` and `C` show the chart page, the object they act on, so that the query's `PENDING` and any failure are seen where they are answered; the map is chosen again to read the chart-centre mark on it (five tests in `GalaxyDisplay.test.tsx` and `App.test.tsx` updated for that). The page selector and the chart's state therefore move up to `GalaxyDisplay`, which already holds the cursor and the centre. Measured as built: at 1280 × 688 the chart page is 798 × 448 px, its stage 798 × 170, the controls 72, the census line 28 and the legend 42; the `SYSTEMS` panel is 400 × 390, its list 82 px (2 rows, scrolling) above a 172 px readout; nothing clips or scrolls but the lists. At 1920 × 1080 the stage is 1,036 × 628 and the list 510 px. With no universe open the second column is empty and `UNIVERSE`, shown whole, says `NO UNIVERSE OPEN`. (2) _The census table folds._ `CENSUS BY LAYER` is a display control led by `DisclosureGlyph`, folded by default, since the page cannot hold both the table and the chart at 1280 px; the census line, the hint and the counts, which the brainstorm requires beside every chart, are always shown. Shown, the table takes the chart's height: the chart's stage keeps a floor of 4 rem instead of the view's 12 rem and the table scrolls past 6 rem, so that the page never clips, and at 1280 × 688 the stage falls to 74 px while the table is open. (3) _The legend is compact and under the chart._ It reads `SYMBOLS NOT TO SCALE`, then `INIT MASS M☉ 0.08` with the five marks and `150`, then the fill, colour and reticle keys with a drawn sample each, as one wrapping row of two lines under the census; one line per band would take five lines the page does not have, and each band's edges are in the census table and each system's mass in the readout. It is not drawn over the canvas: over a 170 px stage it covered the picture. (4) _`OUT` and `OUT OF RANGE`._ The list says `IN RANGE` or `OUT` in a 5 rem column and the readout `IN RANGE` or `OUT OF RANGE`, as T11.e and T11.f ask; both are statements about the chart centre, never `REACHABLE` (D9). (5) _Known by-eye points for T12.b:_ the chart at 1280 × 688 is a 798 × 170 letterbox, and 798 × 74 while the census table is open.
- **Fixed in validation (T11, the triad's clipping).** The implementer's T11 note that the overlay hides 11 px of the triad at 1280 × 688 and 107 px with the census table open was wrong in both figures and in the first case in kind. Re-measured in headless Chromium with the bundled B612, by the same method (the display's DOM from jsdom under the real stylesheet, `document.fonts` checked, the layout viewport verified at exactly 1280 × 688 and 1920 × 1080): with the table folded the stage is 798 × 170 and **nothing is hidden** (the triad's 240 × 128 px box has 42 px of headroom); with the table open the stage is 798 × 74 and **54 px** of the box is clipped, which takes all three direction labels, the north axis's end symbol and part of two axis lines, so the view showed no orientation at all — the guide's "the view always shows an axis triad". Every other figure of ruling (1) and of T8.e was reproduced exactly (chart page 798 × 448, controls 72, census 28, legend 42, `SYSTEMS` 400 × 390, list 82, readout 172; 1,036 × 628 and a 510 px list at 1920 × 1080; nothing clips or scrolls but the lists, at either window), except that the "controls row 72 px" is `ChartControls`; the view's own controls row is 54 px, since it wraps to two lines at 798 px. The fix is general, in the view rather than in the chart's CSS: `furniture.ts` gains `triadBoxRem(viewport)` and `TriadBoxRem`, and `triadLayout` and `triadFootprintPx` take the box, so a stage shorter than 15 × 8 rem gets a triad of the stage's own size with its axes shortened to leave room for a label beyond each end (`axisReachRem`); `AxisTriad` takes `boxRem` as a required prop and `SpatialView` passes it. It is a no-op on any stage with the room for the usual box, so the chart at 1920 × 1080 and every later 3D display are unchanged. Re-measured after the fix: **nothing is hidden past the overlay in any of the four states**, the tightest clearance being `COREWARD`'s 8.15 px, and the only boxes that scroll anywhere are the system list (+14 px at 1280) and the census table (+12 px). The 2 rem toggle takes 4 px from the stage, so the chart's heights now read: at 1280 × 688 stage 166 px folded and 70 px with the table open (the triad's box clamping to 15 × 4.375 rem), census 32 and 128, controls 72, legend 42, page 448; at 1920 × 1080 stage 624 and 528, page 840. Two by-eye points for T12.b: in a 70 px stage the shortened axes put the three end symbols close enough to read as one blob, which is the price of the table and the 688 px window; and at every size `COREWARD`'s label still overprints the north axis's end symbol, which `triadLayout` does not check for (it avoids label-label collisions only) and which predates T11.
- **Fixed in validation (T11.g, the chart's keys on another page).** The pages stay mounted so that none asks the server again, but `SpatialView`'s single keys are a `document` listener (D3), so `T`, `S`, `F`, `O`, `Z`, `+` and `-` turned and zoomed the hidden chart's camera while `PARAMETERS` or `GALAXY MAP` was on show, invisibly: pressing `t` on the map left the chart at `TOP` on return. The chart page's content is now wrapped in `Activity` (D1's mechanism, which T10 already wrote the view for: it tears the effects down and keeps the state), so the view's keys, frames and observers belong to the page on show. Tested in `LocalChartPanel.test.tsx`.
- **Orchestrator's further rulings for the owner, applied in validation (T11).** (6) _`HEIGHT` is the `GALACTIC` frame's z and nothing else._ The guide reserves `RADIUS`, `ANGLE` and `HEIGHT` for that frame's coordinates ("Voice and nomenclature"), and `CURSOR` and the view's furniture row already use `HEIGHT` that way, so `SystemReadout` keeps those words for the system's own place and its T11.f `Z` becomes `HEIGHT`; the offset from the chart's reference plane, which T11.f called `HEIGHT`, becomes `NORTH`, the guide's name for +z, beside the `COREWARD` and `SPINWARD` offsets it belongs with and matching the legend's `FILLED NORTH OF PLANE`. The readout therefore reads `NORTH -3.20 ly` where T11.f wrote `HEIGHT -3.20 ly`, and `Z` is gone from the console. (7) _An answer the client cannot chart is a fault, not a crash._ `layerBands` returns `null` instead of throwing, and the new `chartDataFault(result)` gives the cause in words (`census incomplete` when the census does not list the five layers once each, `query radius unusable` when the radius is not a positive length), which `useLocalChart` exposes as `fault` and withholds the answer for. `LocalChartPanel` reads `CHART DATA INVALID: <cause>` in `--status-caution` with `RETRY`, the galaxy map's `MAP DATA INVALID` path (T8.d) through the same `StatusLine`, and `SYSTEMS` says `CHART DATA INVALID: retry on the LOCAL CHART page`. The wire is decoded but not validated and the renderer has no error boundary, so this is the difference between a reported fault and a blank console. (8) _A chart the link no longer backs is stale, as a kept map picture is._ `useLocalChart` gives `stale` (an answer on show with the link down), and it applies T10's ruling (3) convention: `paint.ts` gains `--text-muted` to its `ColourTokens` and `staleTokens`, which draws every mark, circle and reticle in `--text-muted` and leaves the `--line` grid and the background; `SpatialView` takes a required `stale` prop, paints with that set, names its canvas `Local chart, stale` and puts the guide's trailing `S` in its furniture row; the census, the list and the readout are muted with a trailing `S` in each block, `SYSTEMS` being named `Systems stale`; and the camera's own `AZM`/`ELV` and the controls, which are live, are left alone. `StaleMark` moves from `GalaxyMapView` to `components/StaleMark.tsx`, shared by both. This narrows D4's "data already shown … is not marked stale" to "is kept, and marked stale", which is what the guide's "loss of the server link makes every live value on the console stale at once" asks. (9) _The chart keeps its stage and the census gives way._ Measured with the census table open and the link down at 1280 × 688, the chart's stage was pinned at its 4 rem floor while `ChartControls` grew by the `NO CARRIER` line, so the view's content overflowed its flex box by 10 px and the furniture row overprinted the controls — the scale bar under `NO CARRIER`. Any full-width line that can appear on the page costs the same, and only 8 px were spare, so a smaller table cap was not a fix. `.local-chart` now has a shrink chain instead: the view is `flex: 1 0 auto` and never shrinks below its own content, the controls and the legend are `flex: none`, and `.census-readout` is a column (`.census-readout__row` carries the old wrapping row) whose `.census-readout__by-layer` and the table's scroller inside it are shrinkable to nothing. The deficit is therefore taken from the table, which scrolls and says so. Measured after the fix, at 1280 × 688: with the table folded the stage is 166 px; open, 72 px with the table at 72; open with `NO CARRIER`, the stage holds 64 px and the table gives the 10 px (62); with the hint as well, 44; with the axis note as well — three extra lines at once, the worst state — the view stands at its 210 px minimum, the table at 40, and the furniture row clears the controls by 4 px, with about 40 px of give left. At 1920 × 1080 the stage absorbs it all and the table keeps its 72 px. The `SYSTEMS` panel had no such chain and its list overflowed its box by 8 px into the gap above the readout in those states, so `.system-list__scroll`'s floor is 2 rem, one row, which its position line makes honest. (10) _The census table shows its position and total._ It scrolls at every window size, not only the shortest, so `CensusByLayer` measures the box with `useScrollMetrics` and reads `1-5 of 5` beneath it through `windowRange` and `formatListPosition`, rows are a fixed 1.25 rem so that the position can be read from the offset, and the column headers are sticky, so no row is read without them. The table is `table-layout: fixed` with `ch` column widths, so a new answer never moves the columns, and `EXP`, which the request's limit does not bound, is written to one decimal below 10,000 and in E notation from there (`1.00E4`), the guide's form for a value outside its field's ladder (D7); T11.d's "one decimal" is kept only where it fits.
- **Owner points left open from T11.** (a) The drive range's plane ring is labelled `PLANE 50 ly` (D13), the one place the operator-set value is drawn without `SET`, where the range sphere reads `RANGE 50 ly SET` and the field `DRIVE RANGE SET`. D9 asks that every place that repeats the value say `SET`; either D13's label gains it or D9 is narrowed to the readings that could be taken for measurements. (b) Nothing announces a new answer: the census line, the hint and the counts are plain `p` and `dl` elements, so an operator using a screen reader is told a query started (the `PENDING` in an `output`) and never that it finished or what came back; the same holds for a field refusal raised when the focus has already left the field. T8.f's cursor readout solved this with polite, atomic live regions, at the cost of announcing on every change. Whether the chart should do likewise is a verbosity judgement for the owner. **Both points are now ruled: see the T12.a rulings, the last bullet of this list.**
- **Also fixed in validation (T11).** A field held back with `aria-disabled` now reads as inert, as a held-back `.control` does (`.form-field__input[aria-disabled="true"]`): `QUERY RADIUS` and the two entry fields took a gesture and discarded it with no change of appearance, against the guide's "a control shows … disabled" distinctly. `CENSUS BY LAYER` keeps `.control`'s 2 rem pointer target instead of overriding it with 1.75 rem. `SystemList`'s spacer is `role="presentation"`, so the options stay the listbox's own children. `GALAXY_PAGES` is module-private again (it was exported without TSDoc and used nowhere else). Tests added or tightened: `distanceDecimalsFor` and `formatBandMsun` in `chartModel.test.ts`; `OVER CELL BUDGET` in the census table, for which `aSystemsInRange` gains `overCellBudget` (the status word was named in a test that only checked `OVER LIMIT`); the exact window size and the last row's `aria-posinset` in `SystemList.test.tsx`; the list's rows staying on show while a new query is pending.
- **Deviations in T12.a, as built.** `GalaxyDisplay.integration.test.tsx` renders `App`, not `GalaxyDisplay`, since the script's first and last steps are `F2` and `F1`. The script is a list of step functions and `playTo(stage)` plays it up to a step; each step's `it` calls `playTo` for its own stage and then reads the console, so a step is written once and a failure names the step that broke. Frames and `setTimeout` are faked for the whole script through `test/fakeFramesAndTimeouts.ts`, so the `t` step can let the turn's frames and the angle readouts' 250 ms hold pass instead of racing them. The centre is the point three quarters of the way down the face-on picture and half way across, (32,768, 0, 12) ly with the height typed, which is off the galactic axis so that the named directions are defined; the query radius is the default 50 ly, since the drive range is. The forty systems are 1 to 24 ly out in layers B to E and then sixteen of layer A from 30 ly, the layer-A systems last so that raising the floor to 0.5 M☉ drops a suffix and every system kept keeps the ID and designation it was answered with, as a real server's would; the census line then goes from `COMPLETE ABOVE 0.08` to `COMPLETE ABOVE 0.50` and `SYSTEMS` from 40 to 24. "`Tab` to the list" is one `Tab` from the last `CURSOR` field, the control before it in the tab order, and the step asserts the focus landed on the list. The last step also asserts that a visit to `LINK` and back sends no request again and opens no second socket. **One shared file changed:** `apps/hyperion/vitest.config.mts` gains `testTimeout: 15_000`. The heaviest display tests (the local chart's, and this script's later steps) take about 2 s each with the suite's workers on eight cores, and Vitest's default of 5 s failed them whenever another build shared the machine — a flake, not a finding. Measured: `pnpm test` for the app is 24–26 s on an idle machine and 35–60 s while two lanes build, against the task's one minute.
- **Orchestrator's rulings for the owner, applied with T12.a.** They settle the two points left open from T11 above, and continue the numbering of the T10 rulings. (10) _The plane ring's label says `SET`._ `chartModel.ts`'s `planeLabel` reads `PLANE 50 ly SET`, as `rangeLabel` does. D9 has the drive range say `SET` wherever the value is drawn, and the plane ring draws the same value as the range circle a short distance from it; two curves showing one number, one marked and one not, invites the unmarked one to be read as measured or commanded, which is what D9 exists to prevent. Three characters is cheaper than an exception to the rule. Tested in `chartModel.test.ts` and `LocalChartPanel.test.tsx`. (11) _A query's answer is announced, politely, from the census summary._ `CensusReadout`'s summary row — the census line, the request's state, `SYSTEMS` and `IN RANGE` — is an `output` named `Census` with `aria-atomic="true"`, so an operator who gives a query from the keyboard is told that it started and then what came back, read as one line rather than as a changed number; the semantic element carries `role="status"`, so no `aria-live` attribute is written (`.claude/rules/typescript-dev.md`). `PENDING` is announced as well as the settled answer, matching `GalaxyMapView`'s density reading and `CentreEntry`'s cursor summary, which are polite regions already. The per-layer table, its `CENSUS BY LAYER` control and the system list stay outside the region, and `SpatialView` still holds no live region at all, so the camera does not chatter as it moves (`SpatialView.test.tsx` asserts zero). Tested in `CensusReadout.test.tsx`: the region is an `output`, it is atomic, its text goes from `PENDING` to the census line, and it holds no table and no control. Two notes for T12.b: `RequestStatus`'s own `output` is nested inside the region, since the request's state is part of what the summary says, and the inner region's change resolves to the outer `aria-atomic`, so a real screen reader should read the answer once, not twice; and the summary is now a flex box of its own (`.census-readout__summary`, laying its three parts out as the row did, with the control beside it), which can wrap a line earlier than before where the column is narrowest. (12) _A status line inside another live region does not announce on its own._ The first of those two notes was wrong to leave to the ear: `RequestStatus` renders `StatusLine`, which is itself an `output`, so ruling 11 left a live region nested in a live region, and screen readers do not agree on what that means — some announce twice, some only the inner change, some read the whole atomic outer region. `StatusLine` therefore takes `announce?: boolean`, `true` unless it is told otherwise; `announce={false}` keeps the `output` and its implicit semantics but writes `aria-live="off"`, which is not a region of its own, so the nearest announcing ancestor — the census summary — governs the change and the answer is read once, as one line. `RequestStatus` passes it through and `CensusReadout` sets it. The other call sites take the default and are unchanged, and none of them stands inside a live region: `GalaxyMapView`'s polite region is the cursor density's own `p`, a sibling of its status line, `CentreEntry`'s is a leaf, and `LocalChartPanel`'s `CHART DATA INVALID` (a sixth call site, beside the five of T8.d and T11) is a sibling of the census, not inside it. `SpatialView` still holds no live region at all. Tested in `RequestStatus.test.tsx` (a status line standing on its own carries no `aria-live`, one told not to announce carries `off`) and `CensusReadout.test.tsx` (the summary is the only announcing element in its subtree).
- **Fixed in validation (T12.a).** Each of the twelve steps was checked by breaking what it proves, one mutation at a time in the source and re-running the file: fourteen mutations, thirteen of which failed the step that owns them (and no other, except where a later step stands on an earlier one). The one that survived: the step's "nearest first" claim held with the client's sort removed, because the fixture answered in distance order, so the script now answers farthest first (`systems: answer.systems.toReversed()`, the order a server that walks cells would use) and the step fails without the sort; `wire.test.ts` already covered the sort itself. The wrap left open above is measured, by T8.e's method (the console's DOM from jsdom under the real stylesheet and the bundled B612, in headless Chromium, the layout viewport verified at exactly 1280 × 688 and 1280 × 720, and T11's figures reproduced: chart page 798 × 448, stage 166, controls 72, legend 42): with an answer on show the summary is one line 477 px wide with `CENSUS BY LAYER` beside it and the census row is 32 px, the same geometry as before ruling 11. It costs a line only in its longest state, a refusal (`REJECTED: the interactive queue is full` with `RETRY`), where the row is 82 px against 64 before, the summary wrapping to three lines and the toggle to a fourth; the extra 18 px come off the chart's stage, which stands at 116 px at 1280 × 688, and with the census table open as well the stage takes its 64 px floor and the table's scroller gives the rest. Nothing clips or scrolls in any of those states at either window, so the flex box stands. The screen-reader confirmation of rulings 11 and 12 stays a T12.b item.
- **Reported, not fixed (T12.a, for the orchestrator and the owner).** (a) _How far `aria-live="off"` carries._ Ruling 12's premise is that the silenced inner `output` leaves the census summary governing its changes. In Chromium that holds for every change that inserts or removes the status line — an answer arriving, `PENDING` appearing on a refresh, `RETRY` appearing with a failure — since the change is seen on the unmarked `div` around it, whose nearest live status is the summary's. It may not hold for a change confined to the silenced `output`'s own text, which is `PENDING` giving way to `NO CARRIER` (the header's `LINK` annunciator, a region of its own, says that at the same moment) and one rejection reason replaced by another: Chromium's event generator walks up to the first element carrying a live status and stops there, and that element may be the silenced one. Turning the nested status line into a plain `span` when it does not announce would close the hole and keep the words; it would cost the `output` element the ruling asked to keep. Ruled as written, and left as the brief has it. (b) _`output` holds phrasing content_, and both the census summary and `SystemReadout` (T11.f) wrap a `p`, a `dl` and a `div` in one; `div role="status"` announces the same and is valid markup. A house-wide choice, so not changed here. (c) _One name for the drive range._ Ruling 10 widened `SET` to a fourth reading, and the setting is now `DRIVE RANGE SET` in its field, `RANGE 50 ly SET` on the sphere, `PLANE 50 ly SET` on the ring and `SET RANGE` in the legend and the readout, with `SET` leading in two and trailing in two; the guide's "one name per thing" and its nomenclature list, which holds neither `DRIVE RANGE` nor `SET`, want one form and a guide entry. (d) The ring's label grew four characters, so T12.b should confirm `placeCurveLabels` still places it at `TOP`, where the sphere and the ring project onto each other; `LocalChartPanel.test.tsx` places it in a 400 px view. (e) One arrow key on a map announces three regions (the hidden cursor summary and both maps' `CURSOR DENSITY`, which share a name); pre-existing in T8.f.
- **Orchestrator's rulings 13 to 16, applied after T12.a.** They continue the numbering of the T10 and T12.a rulings and settle four of the five points reported above. (13) _The census live region holds text only, and no control inside it._ Ruling 12's nested-then-silenced `output` is retired rather than bet on: `CensusReadout`'s region now holds the census line, the request's state **in words** and the counts, and `RETRY` and `CENSUS BY LAYER` are its siblings on `.census-readout__row`, `RETRY` described by the state's words (`aria-describedby`), so that, reached outside the region, it still says which failure it retries. `annunciation` and its `Annunciation` type are exported from `components/RequestStatus.tsx` so that the words can be rendered in the region directly — a `p` of the new `.census-readout__state`, which shares `.request-status__text`'s rules (its letter spacing, and `--status-caution` for its `--fault` form) by selector rather than by copy — instead of nesting a component that carries a region of its own. That closes report (a): how far `aria-live="off"` carries in Chromium is no longer asked, and no button _appears_ inside a region either, which was its own spurious announcement. `CensusReadout` was the only caller that ever set `announce`; every other caller took the default — `NewUniverseForm` (one `StatusLine`, one `RequestStatus`), `UniversePanel` (two `RequestStatus`), `GalaxyMapView` (one of each), `ParametersPanel` and `LocalChartPanel` (one each) — and none of them stands inside another live region, so there is no second nested case and **the prop is gone** from `StatusLine` and `RequestStatus` along with ruling 12's test of it: a prop that exists for one retired workaround is worse than no prop. `StatusLine` is now documented as never standing inside another region. Ruling 11's "`RETRY` is what the operator can do about the answer, so it is read with it" is thereby reversed, and its test becomes "keeps the table and both controls out of what is announced", beside "describes RETRY by the failure it retries". Ruling 12's nesting test becomes "holds no region of its own inside the summary", and a **counting** test holds what ruling 12 left in doubt: `NO CARRIER` replacing `PENDING`, a change confined to the state's words, announces from the summary (`announcements`, below); with a silenced `output` put back around the words it reports none. Since the census now renders the state itself, what `RequestStatus` did for it is tested on the census as well: no `RETRY` while `PENDING` or while the link is down, the link's state plain, a timeout in `--status-caution`, a refusal plain. `SpatialView` still holds no live region at all. (14) _`output` is for phrasing content._ Both regions that hold block content become a `div` with `role="status"` and an explicit `aria-atomic="true"`: the census summary and `SystemReadout`, which wraps a `dl`. This is not in tension with the rules' "semantic element before ARIA", which asks for a role only where no native element fits, and an `output` that cannot legally hold a `dl` does not fit; the reasoning is in the code, since the next reader will ask. `role="status"` already implies `aria-atomic="true"` in ARIA, so the attribute is written for the reader rather than for the browser and nothing about what is announced changes; both keep their names (`Census`, `Selected system`) and every test that finds them by role. Each needs one `// oxlint-disable-next-line jsx-a11y/prefer-tag-over-role`, because the rule's suggested tag is the very element the content model forbids. `.system-readout`'s `display: block` goes with the `output`, a `div` being one already. **The audit of the other `output` elements, as ruling 14 asked:** `components/LinkStatus.tsx` holds `LINK_STATUS_LABEL[status]`, one string — correct, unchanged; `components/StatusLine.tsx` holds its `text` — correct, unchanged; those are the only two left, and both are phrasing-only by construction. Report (b) is closed. (15) _One name for the drive range._ Three surface forms stand: `RANGE <n> ly SET` and `PLANE <n> ly SET` on the chart's curves (`chartModel.ts`, unchanged, the chart being the context for the abbreviation), and `DRIVE RANGE` for the field. So `ChartControls`'s `label="DRIVE RANGE SET"` becomes `label="DRIVE RANGE"` — `SET` marks a _drawn value_ as an operator setting rather than a measurement (D9), and a field the operator types into already carries that — `SystemReadout`'s `SET RANGE` becomes `DRIVE RANGE`, and `SymbolLegend` reads `ACCENT WITHIN DRIVE RANGE`. A fifth spelling the ruling's table did not list, `SystemList`'s column head `RANGE` over the same `IN RANGE`/`OUT` words, becomes `DRIVE RANGE` too, since the rule keeps `RANGE` alone for the chart's curve labels (tested in `SystemList.test.tsx`). The within-or-beyond words — `IN RANGE`, the list's `OUT` and the readout's `OUT OF RANGE`, and the census's `IN RANGE` count — are statements about the range and not names of it, as the ruling treats the readout's, so they stand; the list's `OUT` against the readout's `OUT OF RANGE` is T11's ruling (4), which the shared `DRIVE RANGE` heading now puts side by side. The readout's reading draws `rangeWords`, that is `IN RANGE` or `OUT OF RANGE`, a within-or-beyond word and not the setting's value, so it takes no `SET`. `NumberField`'s `name` stays `DRIVE RANGE`, so the refusal still reads `DRIVE RANGE INVALID: enter 0.01 to 500 ly`; its two doc comments are corrected. `label` and `name` now carry the same string for both fields, and both props are kept, since the distinction — the visible label against what a refusal may call the field — survives the day a label needs a word again. Report (c) is closed in the code; the matching **nomenclature entry is still owed in `docs/frontend/ux-guidelines.md`, which is the owner's**, and is deliberately not applied here: `DRIVE RANGE`, a setting, the range a chart counts systems as in against its centre (D9), abbreviated `RANGE` on the chart's curve labels, its trace in the galactic plane `PLANE`, `SET` after every drawn value of it, and `IN RANGE`/`OUT OF RANGE` (`OUT` where the list has no room) for a system's standing against it. Two by-eye points for T12.b: `DRIVE RANGE` is two characters longer than `SET RANGE` and is now the longest label in the readout's first column, whose width is `max-content`; and in the list's 5 rem column the head wraps to two lines — by B612's advance widths at 0.875 rem with 0.15 em tracking, `DRIVE` is 3.15 rem and `RANGE` 3.54 rem, so neither overflows, and the head, already two lines or more for `INIT MASS M☉` in its 6 ch, grows no taller; computed by the UX review, not rendered. (16) _`CURSOR DENSITY` announces once per arrow key._ The cause was **two regions, not two updates**: the cursor is shared by both views and both readings take their horizontal from x (`cursorInView`), so a face-on `ArrowUp` or `ArrowDown` changed the face-on and the edge-on reading at once and each announced. The y keys face-on and the z keys edge-on always changed one reading, so the doubling was the x keys' alone. `CursorDensityReading` therefore takes `announce`, `aria-live="polite"` while the view's own picture has focus and `"off"` otherwise, from a `focused` boolean `MapViewBody` keeps through the canvas's `onFocus` and `onBlur`, and drops during render whenever there is no picture, since a canvas removed while focused fires no `blur` (a change of population does that, and the next picture's reading would otherwise announce unfocused; tested); the arrow keys reach only a focused picture, so the reading that announces is always the one the key asked for, and the other stays on screen to be read on demand. The consequence, recorded: a cursor moved by typing `X`, `Y` or `Z` in `CURSOR`, where no picture has focus, announces the position summary alone and neither density. `CentreEntry`'s hidden position summary is untouched and is a region of its own saying something else, so report (e)'s three regions for one arrow key are now two; whether that is still too chatty is a verbosity judgement, not a defect. Tested by **counting**, not by reading text: `GalaxyDisplay.test.tsx` watches the document with a `MutationObserver` across one `{ArrowDown}` and maps each mutation to the nearest ancestor carrying a live status (`announcingRegion` and `announcements`, in `src/renderer/src/test/liveRegions.ts`; the former yields nothing where that ancestor is turned off, as Chromium's event generator stops at the first live ancestor), then expects exactly one `CURSOR DENSITY` region, the face-on one; it reports two with the fix reverted, one text change in each, which is how the cause was told apart from two updates. A second test counts every region the key announces from and expects two, the face-on reading and the position summary, so report (e) is now held by a test rather than by ear. `GalaxyMapView.test.tsx` holds the unit level: the reading is `polite` once the picture has focus, `off` before it, `off` again once focus leaves (without the `onBlur`, focus that moved to the other view would leave both announcing), and `off` for a new picture after the focused canvas was taken away.
- **P05.T12.b, the automatable part (with rulings 13 to 16).** T12.b is the by-eye and cross-stack pass and most of it needs a person at a running client, so it is not claimed here. What was expressible as a test is done: the two counting tests above; the accessibility list's "the `Selected system` output is announced on selection", held by count in `GalaxyDisplay.integration.test.tsx` — one `ArrowDown` in the list announces from that readout and from nothing else, though the list's rows and its position line and the chart's mark labels change too; and **by-eye point (d)** — `LocalChartPanel.test.tsx` now turns the chart to `TOP` under reduced motion, where the plane ring projects onto the range sphere's outline and the two `SET` labels are drawn at nearly one place, and asserts that `placeCurveLabels` still places all three curve labels in the 400 × 300 view rather than dropping one. It does. What remains for the owner's eyes, unchanged: the whole cross-stack checklist (it needs a real server and a data directory), every "By eye" item (the galaxy's shape, the ramp's floor and ceiling against what is on screen, the background matching `--surface-0`, the grid and stalks, frame rate on a drag), the window and root-font-size sweep at 1280 × 720 and 1920 × 1080 at 80%, 100% and 150% with `prefers-reduced-motion` both ways, the contrast pairings, and the two readings ruling 13 makes cheap but does not remove — that a real screen reader reads a census answer once, as one line, and that one arrow key on a map is heard as one density reading and one position. One more by ear: `C` pressed on a focused map hides the map page with its canvas still focused, and the density reading then relies on Chromium blurring a focused element that is hidden (jsdom does not), or that view's reading stays `polite` until its canvas next takes and loses focus.
- **Ruling 18, applied, with every other place the focus fell to the page (round 6).** The census's `RETRY` now hands the focus, as it is pressed, to `CENSUS BY LAYER` beside it, or, before the first answer, when there is no toggle, to the `LOCAL CHART` tab; never to the document's body. `RETRY` stays after the counts. The ruling's first choice cannot be taken as written. The map canvas `C` was pressed on is never on show while `RETRY` is, since `C` shows the chart's page, which hides the map's, and Chromium does not focus an element it does not display (jsdom does, so a test of that route would pass on a route that does nothing in the app). `C` is in any case a document key, not the canvas's; the element that shows it, `CENTRE CHART`, is on show, but it stands in the other column and re-centres on the cursor rather than sending the failed query again, and a census is also issued from `QUERY RADIUS`, `MIN MASS`, `DRIVE RANGE` and `CHART TIME`. So the ruling's fallback serves, and the page's tab when there is none. The same defect was in seven more places, each fixed to a control that stays: each map view's `RETRY`, after a failed request or `MAP DATA INVALID`, whose body is mounted afresh, to the `GALAXY MAP` tab (not the `POPULATION` option, where an arrow key would ask for both maps again); the `PARAMETERS` `RETRY` to its tab, the page's only control left; the universe list's `RETRY` to the panel's `Universe` title control, as the panel's fold already did; the `CHART DATA INVALID` `RETRY`, which stays on show while its query is pending (the fault is the last answer's) and so keeps the focus, and goes once a usable answer arrives, when the focus it held goes to the `LOCAL CHART` tab; `C`, which hides the page the focus is on when pressed on a map, on the population options or in the parameters list, and now hands the focus to the `LOCAL CHART` tab (a layout effect in `GalaxyPages`, as a choice made on the tabs moves it), and likewise from the tab of the page it leaves; `F1` and `F2`, which hid the display the focus was in, and now hand it to the navigation tab of the display shown (`ConsoleFrame`); and a create left unconfirmed by a lost link, which shows `UNIVERSE` whole over the pages whatever had the focus, and now hands it to the `Universe` title control. A page's tab reaches its controls through `PageTabFocus` (`displays/galaxy/pageTabFocus.ts`), provided by `GalaxyPages`. Checked and already right: `OPEN` and `CREATE` (the fold on success moves the focus to the title control; a refusal leaves them), `DISMISS` (to `CREATE`), `CENTRE CHART` (it stays on show, now tested), the chart's controls and the page tabs. `StatusLine`'s control is now described by its words, so that every `RETRY` says what it retries, as the census's does (the UX review's point). Tested by `document.activeElement` after activation from the keyboard: twelve tests, each red with its fix reverted, and a thirteenth that `CENTRE CHART` keeps the focus.
- **Plan 04's note on the chart centre at 128 pixels, checked and closed (round 6): not a defect in the display.** `GalaxyMapView`'s `pick` passes `pixelToLy` the pointer's fractional column and row, so a pick is where the pointer is; the cursor starts at the origin; a face-on pick returns `[x, y, cursorLy[2]]`; `stepCursor` adds whole pixels to the cursor as it stands; `CENTRE CHART` and `C` publish the cursor unchanged, and `toRangeRequest` carries it exactly. Nothing snaps z to a row's centre: the density _reading_ at z = 0 takes the row below the plane, since a point on a boundary lies in the pixel below, which moves nothing. So the note describes only the server test's own way of choosing a centre. Pinned in `mapCursor.test.ts` ("the chart centre's height", 128-pixel geometry): face-on picks at a pixel's centre, a corner, inside a pixel and beyond the edge, and 640 face-on steps of one and ten pixels into every edge and back, all leave z at +0 by `Object.is`; an edge-on pick on the plane keeps 0 and one at −300 ly keeps −300; edge-on steps return onto the plane. And in `GalaxyDisplay.test.tsx`, "centres the chart on the plane after a face-on click and face-on steps": the query's z cell and offset are both 0. Moving z half a pixel in a face-on pick turns fourteen tests red, and a −0 three. One consequence, recorded and not changed: a pick or step stops at the edge pixel's centre, so edge-on steps that reach the top or bottom row and come back sit on row centres, half a pixel off the plane (−512 ly at 128 pixels). That is the operator stepping on the edge-on map, which sets z by design, and `Z` entered as `0` puts it back.
- **Rulings 13 to 16, adversarially validated (round 6).** Each test the rulings added was broken in the source, one mutation at a time, and each went red: a nested `output` around the census's words, a silenced one (ruling 12's arrangement) and `RequestStatus` nested whole each fail five tests, among them "holds no region of its own inside the summary" and "announces a change confined to the state's words"; `RETRY` or `CENSUS BY LAYER` moved into the region fails "keeps the table and both controls out of what is announced"; `RETRY` without `aria-describedby` fails its description test; the summary or `SystemReadout` turned back into an `output`, and the summary without `aria-atomic`, each fail their test; `DRIVE RANGE SET` on the field, `RANGE` as its refusal name, `SET RANGE` in the readout or the legend, `RANGE` over the list's column and either curve label without `SET` each fail one to four tests; every density reading announcing, no `onBlur`, and no drop of the flag when the picture goes each fail theirs, and a second live region inside the reading fails both counting tests. **`announcements` could count wrongly, three ways, now fixed:** it merged every change a key press made into one set, so a region that changed in two updates counted once (a position summary made to change twice per key passed the old helper and fails the new, which counts each region once per delivery to the observer); it counted regions that are `hidden`, `aria-hidden`, inert or under `Activity`'s `display: none`; and it counted removals, which a region does not announce by default. It does notice two regions in one update, and counts one region changed twice in one update once, as a screen reader hears it. `test/liveRegions.test.tsx` holds each case, the four ways of hiding, an element added hidden and `aria-relevant` among them; the old helper fails eight of its fourteen, and every display test passes under both. **Ruling 16's cause reproduced:** with the fix reverted, one `ArrowDown` on the face-on map is one delivery carrying one `characterData` change in each density reading: two regions, not two updates. **The `C` key:** jsdom never blurs an element it hides, and fires `blur` whenever the focus moves on, so it can neither reproduce nor rule out a density flag left set in Chromium; it can show that the client itself moves the focus. The behaviour is now independent of the blur: `GalaxyPages` moves the focus off a page it hides, so the canvas's own `blur` fires in any browser, and `MapViewBody` reads the focus afresh when `Activity` shows its display again (tested with the `focusout` kept from React, as a browser that took the focus unannounced would leave it). The density tests of `GalaxyMapView.test.tsx` now count announcements instead of reading `aria-live`. For P05.T12.b, in place of the last sentence of the bullet above, by keyboard and by ear: after `C` on a focused map the focus ring is on `LOCAL CHART`, its name is heard, and the census's `PENDING` and answer still follow it; back on the map, one arrow key on either view is heard as one density reading and one position; `F1` and `F2` leave the ring on the tab of the display shown; and, where a failure can be produced, each `RETRY` leaves the ring where the bullet above says — `CENSUS BY LAYER` or `LOCAL CHART` for the census, `LOCAL CHART` once a `CHART DATA INVALID` clears, `GALAXY MAP` for a map, `PARAMETERS` for its page, the `Universe` title control for the list — with the target's name heard. By pointer, `:focus-visible` draws no ring after a click; record whether the next `Tab` continues from the target. Reviews of the rulings' diff (`13fee1d..20268ad`): the UX review found no defect, and three points to consider — the typed cursor announcing no density (recorded under ruling 16 above, a verbosity judgement), the list row's name saying `OUT` rather than `OUT OF RANGE` (T11's ruling (4), left for the owner), and the other `RETRY`s undescribed (fixed above); the TypeScript review's four should-fix findings are fixed — the helper's two, the density tests reading attributes, and the census test with four reasons to fail, now split — except that the two tag checks stay, each in a test of its own, since ruling 14 is a rule of HTML's content models and only the markup can show it kept; the negative class checks now match the whole class list, so that a renamed caution class cannot pass unseen. A second review of this round's own changes found, and this bullet and the one above record as fixed, the `CHART DATA INVALID` `RETRY` moving the focus while it stayed, and the display switch and the forced `UNIVERSE` dropping it.
- **Ruling 28's two fixes, as built (round 7, `ui`).** _Edge-on steps keep their lattice._ An edge-on step that would pass the edge pixels' centres now stops at the furthest height a whole number of pixels from where the cursor was (`heightOnLattice` in `mapCursor.ts`), so as many one-pixel steps back as it moved return it exactly: from the plane at 128 pixels it stops at ±31,744 ly, where it used to land on the edge pixel's centre at ±32,256 ly and steps back ended at z = −512 ly. A height already beyond the limit, as a typed one can be, is still brought to it as a pick is. The test "stops at the centre of the edge pixel" pinned the old behaviour and gives way to two (the centre is still reached where it lies a whole pixel away; a step that cannot be whole is not taken), and new tests show ±40 steps from the plane stopping at ±31,744 ly and a clamp followed by as many steps back as moved being the identity from the plane, from a typed height, from one off the whole pixels, and after a clamped ten-pixel step. Since the plane lies on a row boundary at every resolution, so does every height the arrows reach from it: a step down stops at −31,744 ly, inside the bottom row, but a step up stops at +31,744 ly, the lower edge of the top row, which `pixelIndexAt` puts in the row below, so the arrows from the plane read the top row's density only by a typed height (a UX review's finding, recorded rather than changed, since the ruling asks for the lattice). Face-on steps still stop on the edge pixel's centre, which `GalaxyDisplay.test.tsx` pins twice ("stops at the centre of the map's edge pixel" and the corner's `BELOW FLOOR`): the ruling names the edge-on map, and **whether face-on should follow is for the orchestrator** (it has the same half-pixel offset from the origin). _`SystemList`:_ a row's accessible name ends `OUT OF RANGE` (`rangeName`) while its 5 rem column still shows `OUT` (`rangeWords`), with a test of both.
- **Additions to `spatial/` for plan 14, as built (round 7, `ui`).** All additive, recorded in full under plan 14's Risks: `PathMark` and `AnnulusMark` with the optional `paths` and `annuli` of `SpatialScene`; four new `SymbolShape` values and the `ringed-circle` kind of `SymbolOutline`; `planeFrame`; `SpatialView`'s optional `axes`, passed to `AxisTriad` (a new optional prop) and, through a last optional `shown` parameter of `triadLayout` and `coreArrowLayout`, to the core arrow and the axis note. Plan 05's spatial tests pass with only their imports changed (`drawList.test.ts`, `frame.test.ts`, `SpatialView.test.tsx`); the new tests are appended. The `GALAXY` display's draw lists are byte for byte what they were: a probe outside the suite wrote 1,152 draw lists of chart scenes built by `toScene` (four centres, three radii, two drive ranges, three viewports, eight camera angles, two zooms), each with its triad, core arrow, placed curve labels and three picks, and the four old outlines, to JSON before and after the round, and the two files are identical (SHA-256 `d190693e…`).
- **Ruling 35's cursor fixes, as built (round 7b, `ui`).** _The lattice on both maps (35.1)._ `heightOnLattice` becomes `onLattice` in `mapCursor.ts` and serves every axis a step moves, face-on x and y as well as edge-on z. Its limit is now the map's extent, half-open as the root cube is (lower edges in, upper edges out, plan 01's `in_root_cube`), and no longer the edge pixels' centres: a step that would leave it stops at the furthest point inside it a whole number of pixels on, and as many one-pixel steps back return it exactly. A coordinate outside the limit, typed beyond the map or on its upper edge, is still brought to the edge pixel's centre as a pick would bring it. A step now changes only the axis it moves: it used to pass the whole point through `pickCursor`, which pulled the other face-on coordinate in from an edge pixel's outer half. Picks still clamp to the centres. _Every row reachable (35.2)._ `pixelIndexAt` now gives each pixel its lower edge along each galactic axis and not its upper one, as the cube and its cells do: a point on the edge between two pixels lies in the one on its +x, +y or +z side, where rows used to take the one below. With the half-open limit, each pixel then holds exactly one point of any step lattice, so the arrow keys read every row and column from any start. The first build read an edge outwards, away from the map's middle, with the old open limit; the UX review found that it left the row just north of the plane unread from the plane (a map of N rows has only N − 1 lattice points strictly inside it), so it was replaced. From the plane at 128 pixels a step up stops at +31,744 ly, the top row's lower edge, and reads the top row; a step down stops at the map's bottom edge, −32,768 ly, and reads the bottom row. **The reading at the plane changes:** the cursor on the plane or the galactic axis, a corner of four pixels on plan 04's even-sized maps, now reads the pixel on the +x and +y (face-on) or +z (edge-on) side, so `GalaxyDisplay.test.tsx`'s centre readings move from codes 36 and 20 to 28 and 12. _Tests._ "stops at the centre of the map's edge pixel" becomes "stops a whole pixel inside the root cube's face", X at 49,152.0 for 57,344.0; the corner's `BELOW FLOOR`, the second test ruling 35.1 expected to change, passes as it was. New display tests reach the top and bottom rows' densities from the plane by arrow keys alone and read the edge pixel where a clamped step stops. `mapCursor.test.ts` adds the edge cases of `pixelIndexAt`, every row read in turn from the plane and from three starts off the lattice, every column and row face-on from the centre and from a typed point, and the face-on clamp-and-back identity in all four directions; fifteen of its tests and four of the display tests fail against the round's baseline. _`ColourToken`_ gains `textMuted` for plan 14's orbits and annuli (ruling 35.6), and `ColourTokens`'s own `textMuted` is now that member. _The `GALAXY` draw lists_ are unchanged: a probe outside the suite built 270 draw lists of chart scenes with `toScene` (three centres, three radii, two drive ranges, a selection and a destination, fifteen camera angles), and each equals the one the round's baseline `drawList.ts` builds, field for field.
