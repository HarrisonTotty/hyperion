...
- **Owner points left open from T11.** (a) The drive range's plane ring is labelled `PLANE 50 ly` (D13), the one place the operator-set value is drawn without `SET`, where the range sphere reads `RANGE 50 ly SET` and the field `DRIVE RANGE SET`. D9 asks that every place that repeats the value say `SET`; either D13's label gains it or D9 is narrowed to the readings that could be taken for measurements. (b) Nothing announces a new answer: the census line, the hint and the counts are plain `p` and `dl` elements, so an operator using a screen reader is told a query started (the `PENDING` in an `output`) and never that it finished or what came back; the same holds for a field refusal raised when the focus has already left the field. T8.f's cursor readout solved this with polite, atomic live regions, at the cost of announcing on every change. Whether the chart should do likewise is a verbosity judgement for the owner.
