...
- The orbit map, the three-dimensional spatial display of one system, keeps those conventions and
  ...
  - Orbits are solid `--text-muted` ellipses. They say where a body lies, so they meet the 6:1 that
    ...
    colour alone must not carry the selection. A selected belt, ring or halo has its edges and ticks
    drawn as the selected orbit is. An orbit in `--text-muted` is not stale:
...
| `FOCUS BODY`                                                           | Command                | Centres the orbit map on the selected planet or dwarf planet, or a selected moon's or ring's planet, in its `BODY` frame; held back with `NO PLANET SELECTED`, the planet's kind and state (`PLANET DESTROYED`) or `ORBIT NOT RESOLVED`  |
...
| `ARCH`                                                                 | Label                  | A host's architecture class: `BARREN`, `TERRESTRIAL ONLY`, `COMPACT MULTIPLANET`, `COMPACT WITH COLD GIANT`, `SOLAR-LIKE`, `ECCENTRIC GIANT`, `WARM GIANT`, `HOT JUPITER`, `SUBSTELLAR COMPACT`                                          |
...
| `KIND`                                                                 | Body kind              | A body's kind: `PLANET`, `DWARF PLANET`, `MOON`, `RING`, `ASTEROID BELT`, `KUIPER BELT`, `PROTOPLANETARY DISC`, `DEBRIS DISC` or `UNRESOLVED CONTACT`. `GIANT PLANET`, in the orbit map's legend, is a gas or ice giant's larger mark    |
| `REGULAR`, `GIANT IMPACT`, `CAPTURED`                                  | Moon origin            | A moon formed in its planet's disc, from impact debris, or captured                                                                                                                                                                      |
| `PRESENT`, `DESTROYED`, `UNBOUND`                                      | State                  | A body in place; destroyed (see `CAUSE`); unbound from its system and no longer tracked                                                                                                                                                  |
| `DISPERSED`, `ENGULFED`, `TIDALLY DISRUPTED`                           | Cause                  | A disc dispersed at the end of its lifetime; a body its expanding host swallowed; one whose pericentre fell inside its primary's Roche limit                                                                                             |
| `ROCKY`, `ICY`, `SUB-NEPTUNE`, `ICE GIANT`, `GAS GIANT`                | Planet class           | A planet's `CLASS` by bulk composition; `ROCKY` and `ICY` are also a belt's `COMPOSITION`                                                                                                                                                |
| `MASSIVE`, `DUSTY`                                                     | Ring class             | A ring's `CLASS`: a massive ring system like Saturn's, or the tenuous dust ring every giant has; an unnamed ring is `MASSIVE RING` or `DUSTY RING`. Its `COMPOSITION` is `POROUS ICE` or `ROCK`                                          |
| `INSIDE GIANT ORBIT`, `BETWEEN PLANETS`                                | Belt site              | A belt's `SITE`: inside the orbit of the innermost giant beyond the snow line; in a wide gap between two planets                                                                                                                         |
| `BEYOND PLANETS`, `OUTER DISC`                                         | Belt site              | A belt's `SITE`: beyond the outermost planet; in the outer disc of a host without planets                                                                                                                                                |
| `BELT PROPER`, `SCATTERED DISC`                                        | Label                  | A belt's two components; `SCATTERED DISC` also labels its annulus                                                                                                                                                                        |
| `SIZE INDEX`                                                           | Label                  | The power-law index of a belt's size distribution                                                                                                                                                                                        |
| `DUST LUMINOSITY`                                                      | Label                  | A belt's dust luminosity as a fraction of its host's (`× HOST`), or `NONE`                                                                                                                                                               |
| `COMETS OVER 1 km`, `COMET RATE`                                       | Label                  | The halo's comets over 1 km across; the long-period comets reaching perihelion near the host, in `/yr`. `COMETARY HALO INNER EDGE` labels the halo's inner circle alone                                                                  |
...
| `SHARE OF SYSTEMS`, `POPULATION MASSES`                                | Heading                | Each population's share of the systems, and of the stellar mass; with `IDENTITY`, `MASS`, `MEAN SYSTEM MASSES`, `DISCS`, `BULGE AND BAR`, `NUCLEAR DISC`, `HALO`, `SPIRAL ARMS`, `HISTORY` and `ROTATION`, the `PARAMETERS` groups       |
| `PARAMETERS`, `GALAXY MAP`, `LOCAL CHART`                              | Page                   | The `GALAXY` display's pages                                                                                                                                                                                                             |
| `UNIVERSE`, `NEW UNIVERSE`, `CURSOR`, `SYSTEMS`                        | Panel                  | The `GALAXY` display's panels and regions                                                                                                                                                                                                |
| `CENSUS BY LAYER`, `ORBIT MAP`, `BODIES`, `EVENTS`                     | Panel                  | Regions of the local chart, the `SYSTEM` display and a star's readout                                                                                                                                                                    |
| `CENTRE CHART`                                                         | Command                | Makes the cursor the local chart's centre                                                                                                                                                                                                |
| `OPEN UNIVERSE`, `CREATE`, `DISMISS`, `RETRY`, `RESET`                 | Command                | Opens a universe (its row then reads `OPEN`); creates one from the form; clears a create's report; repeats a failed request; returns the display time to the time the display was opened at                                              |
| `RUN`, `HOLD`                                                          | Mode                   | The display time's `MODE`: running at a rate (`RUN 1 d/s`), or still                                                                                                                                                                     |
| `TOP`, `SIDE`, `FRONT`, `OBLIQUE`                                      | Preset                 | A spatial view's camera; `OBLIQUE` is the default                                                                                                                                                                                        |
| `STABLE ZONE`, `SNOW LINE`, `HABITABLE ZONE`                           | Label                  | A host's zones: its readout rows, the annuli's labels and their switches                                                                                                                                                                 |
| `PENDING`, `TIMED OUT`                                                 | Command state          | Awaiting the server's answer; no answer in time                                                                                                                                                                                          |
| `ESTABLISHING LINK`, `LINK NOMINAL`, `NO CARRIER`, `LINK INCOMPATIBLE` | Status                 | The server link's state                                                                                                                                                                                                                  |
| `NO UNIVERSE OPEN`, `NO SYSTEM SELECTED`, `NO PLANET SELECTED`         | Status                 | Nothing is open or selected to show                                                                                                                                                                                                      |
| `NO BODIES`, `NO SYSTEMS IN MAP`, `NO LIGHT`                           | Status                 | Nothing to draw; a lightless object's luminosity                                                                                                                                                                                         |
| `SYSTEM DATA INVALID`, `BODY DATA INVALID`                             | Fault                  | An answer that failed the display's checks, with the reason and `RETRY`; also `MAP DATA INVALID` and `CHART DATA INVALID`                                                                                                                |
| `INCLUDED`, `OVER LIMIT`, `BELOW MIN MASS`, `OVER CELL BUDGET`         | Census status          | A census layer's `STATUS`; the census line reads `COMPLETE ABOVE 0.5 M☉`                                                                                                                                                                 |
| `GALACTIC`                                                             | Frame                  | The galaxy-wide frame; its readings `RADIUS`, `ANGLE` and `HEIGHT` stand under this heading                                                                                                                                              |
| `COREWARD`, `RIMWARD`, `SPINWARD`, `ANTISPINWARD`, `NORTH`, `SOUTH`    | Direction              | Galactic directions, local to a point                                                                                                                                                                                                    |
| `CHART TIME`                                                           | Setting                | The local chart's universe time                                                                                                                                                                                                          |
| `QUERY RADIUS`                                                         | Setting                | The radius of the sphere the local chart fetches                                                                                                                                                                                         |
| `COLUMN DENSITY`                                                       | Map quantity           | Systems per ly² along the line of sight; `CURSOR DENSITY` is its value at the cursor. Gas is `N(H)`                                                                                                                                      |
| `LOG SCALE`, `FACE-ON FROM NORTH`, `ROTATION COUNTER-CLOCKWISE`        | Label                  | The galaxy map's scale and orientation                                                                                                                                                                                                   |
| `SYMBOLS NOT TO SCALE`, `IN DRIVE RANGE`, `BRACKET SELECTED`           | Legend                 | A spatial display's legend entries                                                                                                                                                                                                       |
...
| `EARLY ASYMPTOTIC GIANT BRANCH`, `POST-ASYMPTOTIC GIANT BRANCH`        | Stellar phase          | The asymptotic giant branch, with `THERMALLY PULSING ASYMPTOTIC GIANT BRANCH` between them, and after it                                                                                                                                 |
| `HELIUM MAIN SEQUENCE`, `HELIUM GIANT BRANCH`                          | Stellar phase          | A stripped helium star's phases, with `HELIUM HERTZSPRUNG GAP` between them                                                                                                                                                              |
| `CARBON-OXYGEN WHITE DWARF`, `OXYGEN-NEON WHITE DWARF`                 | Stellar phase          | A white dwarf's phase by its core, with `HELIUM WHITE DWARF`; `NEUTRON STAR` and `BLACK HOLE` are phases too                                                                                                                             |
| `NO REMNANT`, `SUBSTELLAR`                                             | Stellar phase          | A star that left nothing; a brown dwarf's phase                                                                                                                                                                                          |
| `PRE-MAIN-SEQUENCE STAR`, `WOLF-RAYET STAR`, `HOT SUBDWARF`            | Object kind            | With `PROTOSTAR`, `DWARF`, `SUBGIANT`, `GIANT`, `SUPERGIANT`, `WHITE DWARF`, `NEUTRON STAR`, `BLACK HOLE` and `BROWN DWARF`, a star's kind as the symbol table draws it                                                                  |
| `EVENTS`, `VARIABILITY`                                                | Label                  | A star's events (`FLARE`, `GLITCH`, `MAGNETAR BURST`, `MAGNETAR GIANT FLARE`, `FU ORIONIS OUTBURST`, `GIANT ERUPTION`, `THERMAL PULSE`), and its variable-star class in words (`DELTA SCUTI`, `CLASSICAL CEPHEID`)                       |
| `STAR A`, `A AND B`                                                    | Designation            | A system's star by letter, and a list of stars as a note reads it                                                                                                                                                                        |
| `UTC`                                                                  | Time system            | The header strip's wall clock                                                                                                                                                                                                            |
