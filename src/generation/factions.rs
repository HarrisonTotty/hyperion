...
impl FactionGenerator {
    ...
    fn generate_faction(
        ...
        let tech_level = self.rng.random_range(1..=10);
        let military_strength = self.rng.random_range(1..=10);
        let economic_power = self.rng.random_range(1..=10);

        // Assign territories
        let num_territories = self.rng.random_range(1..=10);
        let mut territories = Vec::new();
        if !available_territories.is_empty() {
            for _ in 0..num_territories.min(available_territories.len()) {
                let idx = self.rng.random_range(0..available_territories.len());
    ...
    fn generate_faction_name(&mut self) -> String {
        ...
        let core = cores[self.rng.random_range(0..cores.len())];
    ...
    fn generate_traits(&mut self) -> Vec<FactionTrait> {
        ...
        let num_traits = self.rng.random_range(2..=4);
        let mut traits = Vec::new();

        for _ in 0..num_traits {
            let trait_idx = self.rng.random_range(0..all_traits.len());
    ...
    fn calculate_relationship(
        ...
        for trait_a in &a.traits {
            for trait_b in &b.traits {
                ...
                // Special cases
                if matches!(trait_a, FactionTrait::Xenophobic)
                    && matches!(trait_b, FactionTrait::Xenophilic)
                {
                    score -= 3;
                }

                if matches!(trait_a, FactionTrait::Militaristic)
                    && matches!(trait_b, FactionTrait::Pacifist)
                {
                    score -= 2;
                }
            }
        ...
        // Random factor
        score += self.rng.random_range(-2..=2);
