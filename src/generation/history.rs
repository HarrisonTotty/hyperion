...
impl HistoryGenerator {
    ...
    pub fn generate_history(
        ...
        for year in (1..=years).rev() {
            let year_events = self.rng.random_range(0..=3); // 0-3 events per year
    ...
    fn generate_first_contacts(
        ...
        for i in 0..factions.len() {
            for j in (i + 1)..factions.len() {
                ...
                let year = -(self.rng.random_range(max_year / 2..max_year) as i32);
                ...
                let event = HistoricalEvent {
                    ...
                    relationship_changes: vec![(factions[i].id.clone(), factions[j].id.clone(), 1)],
                };
    ...
    fn generate_random_event(
        ...
        let idx1 = self.rng.random_range(0..factions.len());
        let mut idx2 = self.rng.random_range(0..factions.len());
        while idx2 == idx1 {
            idx2 = self.rng.random_range(0..factions.len());
        }
        ...
        let (description, relationship_change) = match event_type {
            ...
            EventType::AllianceDissolved => (
                format!(
                    "Alliance between {} and {} dissolved",
            ...
            EventType::Incident => (
                format!(
                    "Diplomatic incident between {} and {}",
                    faction1.name, faction2.name
                ),
                -1,
            ),
            EventType::CulturalExchange => (
                format!(
                    "{} and {} initiate cultural exchange",
                    faction1.name, faction2.name
                ),
                1,
            ),
    ...
    pub fn generate_timeline_summary(
        &self,
        ...
        let mut decades: std::collections::BTreeMap<i32, Vec<&HistoricalEvent>> =
            std::collections::BTreeMap::new();
        for event in events {
            let decade = (event.year / 10) * 10;
            decades.entry(decade).or_default().push(event);
        }
...
mod tests {
    ...
    fn test_first_contact_events() {
        ...
        let first_contacts: Vec<_> = history
            .iter()
    ...
    fn test_event_types() {
        ...
        let has_trade = history
            .iter()
            .any(|e| e.event_type == EventType::TradeAgreement);
