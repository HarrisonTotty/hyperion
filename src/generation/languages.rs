...
impl AlienLanguage {
    ...
    fn generate_phonology(rng: &mut StdRng) -> Phonology {
        ...
        let all_consonants = vec![
            "p", "t", "k", "b", "d", "g", "m", "n", "f", "s", "h", "v", "z", "l", "r", "w", "y",
            "ch", "sh", "th", "zh", "kh", "gh",
        ];

        // Select subset of consonants
        let num_consonants = rng.random_range(8..16);
        let mut consonants = Vec::new();
        for _ in 0..num_consonants {
            let idx = rng.random_range(0..all_consonants.len());
        ...
        let num_vowels = rng.random_range(3..7);
        let mut vowels = Vec::new();
        for _ in 0..num_vowels {
            let idx = rng.random_range(0..all_vowels.len());
        ...
        Phonology {
            consonants,
            vowels,
            consonant_clusters: rng.random_bool(0.5),
            final_consonants: rng.random_bool(0.7),
        }
    }

    fn generate_structure(rng: &mut StdRng) -> WordStructure {
        let min_syllables = rng.random_range(1..=2);
        let max_syllables = rng.random_range(min_syllables..=4);
    ...
    fn generate_word(rng: &mut StdRng, phonology: &Phonology, structure: &WordStructure) -> String {
        let num_syllables = rng.random_range(structure.min_syllables..=structure.max_syllables);
        let mut word = String::new();

        for i in 0..num_syllables {
            let syllable =
                Self::generate_syllable(rng, phonology, structure.pattern, i == num_syllables - 1);
    ...
    fn generate_syllable(
        ...
        match pattern {
            SyllablePattern::CV => {
                // Consonant + Vowel
                let c = &phonology.consonants[rng.random_range(0..phonology.consonants.len())];
                let v = &phonology.vowels[rng.random_range(0..phonology.vowels.len())];
                syllable.push_str(c);
                syllable.push_str(v);
            }
            SyllablePattern::CVC => {
                // Consonant + Vowel + Consonant
                let c1 = &phonology.consonants[rng.random_range(0..phonology.consonants.len())];
                let v = &phonology.vowels[rng.random_range(0..phonology.vowels.len())];
                syllable.push_str(c1);
                syllable.push_str(v);

                if phonology.final_consonants || !is_final {
                    let c2 = &phonology.consonants[rng.random_range(0..phonology.consonants.len())];
