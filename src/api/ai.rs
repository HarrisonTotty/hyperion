...
#[get("/v1/ai/ships")]
pub fn list_ai_ships(world: &State<SharedGameWorld>) -> Json<Vec<AIShipResponse>> {
    ...
    let ships: Vec<AIShipResponse> = ship_ids
        ...
        .filter_map(|ship_id| {
            world
                .ai_manager
                .get_personality(*ship_id)
                .map(|personality| AIShipResponse {
                    ship_id: *ship_id,
                    personality,
                })
        })
...
pub fn create_ai_ship(
    ...
    if let Some(route) = &request.patrol_route {
        world
            .ai_manager
            .set_patrol_route(request.ship_id, route.clone());
    }
...
pub fn delete_ai_ship(ship_id: String, world: &State<SharedGameWorld>) -> Option<()> {
    let ship_id = Uuid::parse_str(&ship_id).ok()?;
    let world = world.read().unwrap();
...
pub fn set_patrol_route(
    ...
    world
        .ai_manager
        .set_patrol_route(ship_id, request.waypoints.clone());
...
pub fn add_hostile_faction(
    ...
    world
        .ai_manager
        .add_hostile_faction(ship_id, request.faction.clone());
...
mod tests {
    ...
    fn create_test_client() -> Client {
        ...
        let rocket = rocket::build().manage(world).mount("/", routes());
    ...
    fn test_set_patrol_route() {
        ...
        let patrol_request = PatrolRouteRequest {
            waypoints: vec![[0.0, 0.0, 0.0], [100.0, 0.0, 0.0], [100.0, 100.0, 0.0]],
        };
    ...
    fn test_delete_ai_ship() {
        ...
        let response = client
            .delete(format!("/v1/ai/ships/{}", ship_id))
            .dispatch();
        assert_eq!(response.status(), Status::Ok);
