...
pub enum SubscriptionRequest {
    ...
    SubscribeToShip { ship_id: Uuid },

    /// Subscribe to all events for a player's ships
    SubscribeToPlayer { player_id: Uuid },

    /// Subscribe to all simulation events
    SubscribeToSimulation,

    /// Unsubscribe from a ship
    UnsubscribeFromShip { ship_id: Uuid },

    /// Unsubscribe from player
    UnsubscribeFromPlayer { player_id: Uuid },
...
impl WebSocketManager {
    ...
    pub fn should_receive_event(&self, client_id: &str, event: &GameEvent) -> bool {
        ...
        if subscription.subscribe_to_simulation && matches!(event, GameEvent::SimulationTick { .. })
        {
            return true;
        }
        ...
        let event_ship_id = match event {
            ...
            GameEvent::ContactDetected {
                detecting_ship_id, ..
            } => Some(*detecting_ship_id),
            GameEvent::ContactLost {
                detecting_ship_id, ..
            } => Some(*detecting_ship_id),
