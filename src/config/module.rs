...
impl ModuleSlot {
    ...
    pub fn validate(&self) -> Result<(), String> {
        ...
        if self.base_cost < 0 {
            return Err(format!(
                "Module slot {} cannot have negative base_cost",
                self.id
            ));
        }
        if self.max_slots < 1 {
            return Err(format!(
                "Module slot {} must allow at least 1 slot (max_slots >= 1)",
                self.id
            ));
        }
        if self.base_hp < 0 {
            return Err(format!(
                "Module slot {} cannot have negative base_hp",
                self.id
            ));
        }
        if self.base_weight < 0 {
            return Err(format!(
                "Module slot {} cannot have negative base_weight",
                self.id
            ));
        }
        Ok(())
...
impl ModuleVariant {
    ...
    pub fn validate(&self) -> Result<(), String> {
        ...
        if self.cost < 0 {
            return Err(format!(
                "Module variant {} cannot have negative cost",
                self.id
            ));
        }
        Ok(())
    ...
    pub fn validate_numeric_ranges(&self) -> Result<(), String> {
        ...
        if self.additional_hp < 0 {
            return Err(format!(
                "Module variant {} cannot have negative additional_hp",
                self.id
            ));
        }
        if self.additional_power_consumption < 0.0 {
            return Err(format!(
                "Module variant {} cannot have negative additional_power_consumption",
                self.id
            ));
        }
        if self.additional_heat_generation < 0.0 {
            return Err(format!(
                "Module variant {} cannot have negative additional_heat_generation",
                self.id
            ));
        }
        if self.additional_weight < 0 {
            return Err(format!(
                "Module variant {} cannot have negative additional_weight",
                self.id
            ));
        }
        ...
        if self.cost > 100000 {
            warn!(
                "Module variant {} has very high cost: {}",
                self.id, self.cost
            );
        }
        if self.additional_weight > 1000000 {
            warn!(
                "Module variant {} has very high weight: {}",
                self.id, self.additional_weight
            );
        }
    ...
    pub fn validate_type_specific_fields(&self) -> Result<(), String> {
        match self.module_type.as_str() {
            ...
            _ => {
                warn!(
                    "Unknown module type '{}' for variant {}",
    ...
    fn validate_power_core_fields(&self) -> Result<(), String> {
        let energy_production = self.stats.get_i64("energy_production").ok_or_else(|| {
            format!(
                "Power core {} missing required field 'energy_production'",
                self.id
            )
        })?;
        let energy_capacity = self.stats.get_i64("energy_capacity").ok_or_else(|| {
            format!(
                "Power core {} missing required field 'energy_capacity'",
                self.id
            )
        })?;
        ...
        if energy_production <= 0 {
            return Err(format!(
                "Power core {} must have positive energy_production",
                self.id
            ));
        }
        if energy_capacity <= 0 {
            return Err(format!(
                "Power core {} must have positive energy_capacity",
                self.id
            ));
        }
    ...
    fn validate_impulse_engine_fields(&self) -> Result<(), String> {
        let max_thrust = self.stats.get_i64("max_thrust").ok_or_else(|| {
            format!(
                "Impulse engine {} missing required field 'max_thrust'",
                self.id
            )
        })?;
        ...
        if max_thrust <= 0 {
            return Err(format!(
                "Impulse engine {} must have positive max_thrust",
                self.id
            ));
        }
    ...
    fn validate_maneuvering_thruster_fields(&self) -> Result<(), String> {
        let angular_thrust = self.stats.get_i64("angular_thrust").ok_or_else(|| {
            format!(
                "Maneuvering thruster {} missing required field 'angular_thrust'",
                self.id
            )
        })?;
        ...
        if angular_thrust <= 0 {
            return Err(format!(
                "Maneuvering thruster {} must have positive angular_thrust",
                self.id
            ));
        }
    ...
    fn validate_shield_generator_fields(&self) -> Result<(), String> {
        let max_shield_strength = self.stats.get_i64("max_shield_strength").ok_or_else(|| {
            format!(
                "Shield generator {} missing required field 'max_shield_strength'",
                self.id
            )
        })?;
        let shield_recharge_rate = self.stats.get_i64("shield_recharge_rate").ok_or_else(|| {
            format!(
                "Shield generator {} missing required field 'shield_recharge_rate'",
                self.id
            )
        })?;
        ...
        if max_shield_strength <= 0 {
            return Err(format!(
                "Shield generator {} must have positive max_shield_strength",
                self.id
            ));
        }
        if shield_recharge_rate <= 0 {
            return Err(format!(
                "Shield generator {} must have positive shield_recharge_rate",
                self.id
            ));
        }
    ...
    fn validate_comms_system_fields(&self) -> Result<(), String> {
        let comm_range = self.stats.get_i64("comm_range").ok_or_else(|| {
            format!(
                "Communications system {} missing required field 'comm_range'",
                self.id
            )
        })?;
        let encryption_lvl = self.stats.get_i64("encryption_lvl").ok_or_else(|| {
            format!(
                "Communications system {} missing required field 'encryption_lvl'",
                self.id
            )
        })?;
        ...
        if comm_range <= 0 {
            return Err(format!(
                "Communications system {} must have positive comm_range",
                self.id
            ));
        }
        if !(1..=10).contains(&encryption_lvl) {
            return Err(format!(
                "Communications system {} encryption_lvl must be between 1 and 10",
                self.id
            ));
        }
    ...
    fn validate_cooling_system_fields(&self) -> Result<(), String> {
        let maximum_coolant = self.stats.get_i64("maximum_coolant").ok_or_else(|| {
            format!(
                "Cooling system {} missing required field 'maximum_coolant'",
                self.id
            )
        })?;
        let generated_cooling = self.stats.get_i64("generated_cooling").ok_or_else(|| {
            format!(
                "Cooling system {} missing required field 'generated_cooling'",
                self.id
            )
        })?;
        ...
        if maximum_coolant <= 0 {
            return Err(format!(
                "Cooling system {} must have positive maximum_coolant",
                self.id
            ));
        }
        if generated_cooling <= 0 {
            return Err(format!(
                "Cooling system {} must have positive generated_cooling",
                self.id
            ));
        }
    ...
    fn validate_sensor_array_fields(&self) -> Result<(), String> {
        let scan_range = self.stats.get_i64("scan_range").ok_or_else(|| {
            format!(
                "Sensor array {} missing required field 'scan_range'",
                self.id
            )
        })?;
        let detail_level = self.stats.get_i64("detail_level").ok_or_else(|| {
            format!(
                "Sensor array {} missing required field 'detail_level'",
                self.id
            )
        })?;
        let scan_time = self.stats.get_f64("scan_time").ok_or_else(|| {
            format!(
                "Sensor array {} missing required field 'scan_time'",
                self.id
            )
        })?;
        ...
        if scan_range <= 0 {
            return Err(format!(
                "Sensor array {} must have positive scan_range",
                self.id
            ));
        }
        if !(1..=10).contains(&detail_level) {
            return Err(format!(
                "Sensor array {} detail_level must be between 1 and 10",
                self.id
            ));
        }
        if scan_time <= 0.0 {
            return Err(format!(
                "Sensor array {} must have positive scan_time",
                self.id
            ));
        }
    ...
    fn validate_stealth_system_fields(&self) -> Result<(), String> {
        let detectability_reduction =
            self.stats
                .get_f64("detectability_reduction")
                .ok_or_else(|| {
                    format!(
                        "Stealth system {} missing required field 'detectability_reduction'",
                        self.id
                    )
                })?;
        let scan_time_increase = self.stats.get_f64("scan_time_increase").ok_or_else(|| {
            format!(
                "Stealth system {} missing required field 'scan_time_increase'",
                self.id
            )
        })?;

        if !(0.0..=1.0).contains(&detectability_reduction) {
            return Err(format!(
                "Stealth system {} detectability_reduction must be between 0.0 and 1.0",
                self.id
            ));
        }
        if scan_time_increase < 0.0 {
            return Err(format!(
                "Stealth system {} must have non-negative scan_time_increase",
                self.id
            ));
        }
    ...
    fn validate_aux_support_system_fields(&self) -> Result<(), String> {
        let hp_regained = self.stats.get_i64("hp_regained").ok_or_else(|| {
            format!(
                "Auxiliary support system {} missing required field 'hp_regained'",
                self.id
            )
        })?;
        let energy_restored = self.stats.get_i64("energy_restored").ok_or_else(|| {
            format!(
                "Auxiliary support system {} missing required field 'energy_restored'",
                self.id
            )
        })?;
        let heat_dissipated = self.stats.get_i64("heat_dissipated").ok_or_else(|| {
            format!(
                "Auxiliary support system {} missing required field 'heat_dissipated'",
                self.id
            )
        })?;
        let num_uses = self.stats.get_i64("num_uses").ok_or_else(|| {
            format!(
                "Auxiliary support system {} missing required field 'num_uses'",
                self.id
            )
        })?;
        ...
        if hp_regained < 0 {
            return Err(format!(
                "Auxiliary support system {} cannot have negative hp_regained",
                self.id
            ));
        }
        if energy_restored < 0 {
            return Err(format!(
                "Auxiliary support system {} cannot have negative energy_restored",
                self.id
            ));
        }
        if heat_dissipated < 0 {
            return Err(format!(
                "Auxiliary support system {} cannot have negative heat_dissipated",
                self.id
            ));
        }
        if num_uses <= 0 {
            return Err(format!(
                "Auxiliary support system {} must have positive num_uses",
                self.id
            ));
        }
    ...
    fn validate_warp_jump_core_fields(&self) -> Result<(), String> {
        let warp_type = self.stats.get_string("warp_type").ok_or_else(|| {
            format!(
                "Warp/jump core {} missing required field 'warp_type'",
                self.id
            )
        })?;
        let warp_delay = self.stats.get_f64("warp_delay").ok_or_else(|| {
            format!(
                "Warp/jump core {} missing required field 'warp_delay'",
                self.id
            )
        })?;
        ...
        if warp_type != "warp" && warp_type != "jump" {
            return Err(format!(
                "Warp/jump core {} warp_type must be 'warp' or 'jump'",
                self.id
            ));
        }
        if warp_delay <= 0.0 {
            return Err(format!(
                "Warp/jump core {} must have positive warp_delay",
                self.id
            ));
        }
        ...
        if warp_type == "jump" {
            let jump_distance = self.stats.get_i64("jump_distance").ok_or_else(|| {
                format!(
                    "Jump core {} missing required field 'jump_distance'",
                    self.id
                )
            })?;
            if jump_distance <= 0 {
                return Err(format!(
                    "Jump core {} must have positive jump_distance",
                    self.id
                ));
            }
        }
    ...
    fn validate_de_weapon_fields(&self) -> Result<(), String> {
        ...
        let recharge_time = self.stats.get_f64("recharge_time").ok_or_else(|| {
            format!(
                "DE weapon {} missing required field 'recharge_time'",
                self.id
            )
        })?;
        let max_range = self
        ...
        let projectile_speed = self.stats.get_i64("projectile_speed").ok_or_else(|| {
            format!(
                "DE weapon {} missing required field 'projectile_speed'",
                self.id
            )
        })?;
        ...
        if recharge_time <= 0.0 {
            return Err(format!(
                "DE weapon {} must have positive recharge_time",
                self.id
            ));
        }
        if max_range <= 0 {
            return Err(format!(
                "DE weapon {} must have positive max_range",
                self.id
            ));
        }
        if projectile_speed <= 0 {
            return Err(format!(
                "DE weapon {} must have positive projectile_speed",
                self.id
            ));
        }
    ...
    fn validate_kinetic_weapon_fields(&self) -> Result<(), String> {
        let _ammo_type = self.stats.get_string("ammo_type").ok_or_else(|| {
            format!(
                "Kinetic weapon {} missing required field 'ammo_type'",
                self.id
            )
        })?;
        let _ammo_size = self.stats.get_string("ammo_size").ok_or_else(|| {
            format!(
                "Kinetic weapon {} missing required field 'ammo_size'",
                self.id
            )
        })?;
        let reload_time = self.stats.get_f64("reload_time").ok_or_else(|| {
            format!(
                "Kinetic weapon {} missing required field 'reload_time'",
                self.id
            )
        })?;
        let num_projectiles = self.stats.get_i64("num_projectiles").ok_or_else(|| {
            format!(
                "Kinetic weapon {} missing required field 'num_projectiles'",
                self.id
            )
        })?;
        let ammo_consumed = self.stats.get_i64("ammo_consumed").ok_or_else(|| {
            format!(
                "Kinetic weapon {} missing required field 'ammo_consumed'",
                self.id
            )
        })?;
        let accuracy = self.stats.get_f64("accuracy").ok_or_else(|| {
            format!(
                "Kinetic weapon {} missing required field 'accuracy'",
                self.id
            )
        })?;
        let effective_range = self.stats.get_i64("effective_range").ok_or_else(|| {
            format!(
                "Kinetic weapon {} missing required field 'effective_range'",
                self.id
            )
        })?;
        ...
        if reload_time <= 0.0 {
            return Err(format!(
                "Kinetic weapon {} must have positive reload_time",
                self.id
            ));
        }
        if num_projectiles <= 0 {
            return Err(format!(
                "Kinetic weapon {} must have positive num_projectiles",
                self.id
            ));
        }
        if ammo_consumed <= 0 {
            return Err(format!(
                "Kinetic weapon {} must have positive ammo_consumed",
                self.id
            ));
        }
        if accuracy <= 0.0 || accuracy > 1.0 {
            return Err(format!(
                "Kinetic weapon {} accuracy must be between 0.0 and 1.0",
                self.id
            ));
        }
        if effective_range <= 0 {
            return Err(format!(
                "Kinetic weapon {} must have positive effective_range",
                self.id
            ));
        }
    ...
    fn validate_missile_launcher_fields(&self) -> Result<(), String> {
        let reload_time = self.stats.get_f64("reload_time").ok_or_else(|| {
            format!(
                "Missile launcher {} missing required field 'reload_time'",
                self.id
            )
        })?;
        let missile_volley_delay = self.stats.get_f64("missile_volley_delay").ok_or_else(|| {
            format!(
                "Missile launcher {} missing required field 'missile_volley_delay'",
                self.id
            )
        })?;
        let num_launched = self.stats.get_i64("num_launched").ok_or_else(|| {
            format!(
                "Missile launcher {} missing required field 'num_launched'",
                self.id
            )
        })?;
        let ammo_capacity = self.stats.get_i64("ammo_capacity").ok_or_else(|| {
            format!(
                "Missile launcher {} missing required field 'ammo_capacity'",
                self.id
            )
        })?;
        ...
        if reload_time <= 0.0 {
            return Err(format!(
                "Missile launcher {} must have positive reload_time",
                self.id
            ));
        }
        if missile_volley_delay < 0.0 {
            return Err(format!(
                "Missile launcher {} cannot have negative missile_volley_delay",
                self.id
            ));
        }
        if num_launched <= 0 {
            return Err(format!(
                "Missile launcher {} must have positive num_launched",
                self.id
            ));
        }
        if ammo_capacity <= 0 {
            return Err(format!(
                "Missile launcher {} must have positive ammo_capacity",
                self.id
            ));
        }
    ...
    fn validate_radial_emission_system_fields(&self) -> Result<(), String> {
        let max_pulse_range = self.stats.get_i64("max_pulse_range").ok_or_else(|| {
            format!(
                "Radial emission system {} missing required field 'max_pulse_range'",
                self.id
            )
        })?;
        let pulse_speed = self.stats.get_i64("pulse_speed").ok_or_else(|| {
            format!(
                "Radial emission system {} missing required field 'pulse_speed'",
                self.id
            )
        })?;
        ...
        if max_pulse_range <= 0 {
            return Err(format!(
                "Radial emission system {} must have positive max_pulse_range",
                self.id
            ));
        }
        if pulse_speed <= 0 {
            return Err(format!(
                "Radial emission system {} must have positive pulse_speed",
                self.id
            ));
        }
...
"#;
        ...
        let slot: ModuleSlot =
            serde_yaml::from_str(yaml).expect("Failed to deserialize ModuleSlot");
...
"#;
        ...
        let fields = variant
            .as_power_core()
            .expect("Should extract power core fields");
...
"#;
        ...
        let fields = variant
            .as_impulse_engine()
            .expect("Should extract impulse engine fields");
...
"#;
        ...
        let variant: ModuleVariant = serde_yaml::from_str(yaml).expect("Failed to parse");
        let fields = variant
            .as_maneuvering_thruster()
            .expect("Should extract thruster fields");
...
"#;
        ...
        let fields = variant
            .as_shield_generator()
            .expect("Should extract shield fields");
...
"#;
        ...
        let fields = variant
            .as_comms_system()
            .expect("Should extract comms fields");
...
"#;
        ...
        let variant: ModuleVariant = serde_yaml::from_str(yaml).expect("Failed to parse");
        let fields = variant
            .as_aux_support_system()
            .expect("Should extract aux system fields");
...
"#;
        ...
        let fields = variant
            .as_warp_jump_core()
            .expect("Should extract warp core fields");
...
"#;
        ...
        let fields = variant
            .as_warp_jump_core()
            .expect("Should extract jump core fields");
...
"#;
        ...
        assert!(
            variant
                ...
                .unwrap_err()
                .contains("negative additional_hp")
        );
    }
...
"#;
        ...
        assert!(
            variant
                .validate_numeric_ranges()
                .unwrap_err()
                .contains("negative additional_power_consumption")
        );
    }
...
"#;
        ...
        assert!(
            result
                .unwrap_err()
                .contains("missing required field 'energy_capacity'")
        );
    }
...
"#;
        ...
        assert!(
            result
                .unwrap_err()
                .contains("must have positive energy_production")
        );
    }
...
"#;
        ...
        assert!(
            result
                .unwrap_err()
                .contains("encryption_lvl must be between 1 and 10")
        );
    }
...
"#;
        ...
        assert!(
            result
                .unwrap_err()
                .contains("detail_level must be between 1 and 10")
        );
    }
...
"#;
        ...
        assert!(
            result
                .unwrap_err()
                .contains("accuracy must be between 0.0 and 1.0")
        );
    }
...
"#;
        ...
        assert!(
            result
                .unwrap_err()
                .contains("detectability_reduction must be between 0.0 and 1.0")
        );
    }
}
