...
pub use ai::AIConfig;
pub use bonus::{BonusConfig, BonusFormat, BonusMetadata, CategoryMetadata, FormattedBonus};
pub use faction_gen::FactionGenConfig;
pub use game_settings::GameSettings;
pub use map::MapConfig as ProceduralMapConfig;
pub use module::{
    AuxSupportSystemFields, CommsSystemFields, CoolingSystemFields, ImpulseEngineFields,
    ManeuveringThrusterFields, ModuleConfig, ModuleSlot, ModuleStats, ModuleVariant,
    PowerCoreFields, SensorArrayFields, ShieldGeneratorFields, StealthSystemFields,
    WarpJumpCoreFields, WarpType,
};
pub use ship_class::{ShipClassConfig, ShipClassRole, ShipSize};
pub use simulation::SimulationConfig as ProceduralSimConfig;
pub use weapon::{
    AmmunitionConfig, KineticWeaponKind, StatusEffectConfig, WeaponConfig, WeaponTagConfig,
};
...
impl GameConfig {
    ...
    pub fn load_from_directory(data_dir: &Path) -> Result<Self, String> {
        ...
        let ai_behavior =
            Self::load_yaml_optional::<AIConfig>(data_dir.join("ai.yaml")).unwrap_or_default();
        let procedural_map = Self::load_yaml_optional::<ProceduralMapConfig>(
            data_dir.join("procedural_generation.yaml"),
        )
        .unwrap_or_default();
        let simulation_params =
            Self::load_yaml_optional::<ProceduralSimConfig>(data_dir.join("simulation.yaml"))
                .unwrap_or_default();
        let faction_generation =
            Self::load_yaml_optional::<FactionGenConfig>(data_dir.join("faction_generation.yaml"))
                .unwrap_or_default();
        ...
        info!(
            "Loaded game settings: team_starting_credits={}",
            game_settings.team_starting_credits
        );
        ...
        let module_variants =
            Self::load_module_variants(data_dir.join("modules"), Some(&module_slots))?;
    ...
    fn load_ammunition(data_dir: PathBuf) -> Result<Vec<AmmunitionConfig>, String> {
        ...
        for category in &categories {
            ...
            if !category_dir.exists() {
                debug!(
                    "Ammo category directory does not exist: {}",
        ...
        info!(
            "Loaded {} ammunition types from {} categories",
            items.len(),
            categories.len()
        );
        Ok(items)
        ...
        for entry in entries {
            ...
            if path.is_file() && path.extension().and_then(|s| s.to_str()) == Some("yaml") {
                ...
                if slots.contains_key(&slot.id) {
                    return Err(format!(
                        "Duplicate module slot ID '{}' found in {:?}",
                        slot.id, path
                    ));
                }
    ...
    fn load_module_variants(
            ...
            for entry in entries {
                    ...
                    && path.extension().and_then(|s| s.to_str()) == Some("yaml")
                {
                    ...
                    if let Some(slots) = module_slots
                        && !slots.contains_key(&variant.module_type)
                    {
                        return Err(format!(
        ...
        for (module_type, variants) in &variants_by_type {
            info!(
                "Loaded {} variants for module type '{}'",
                variants.len(),
                module_type
            );
        }
...
"#;
        ...
        assert!(
            template.is_ok(),
            "Failed to parse module with slot metadata"
        );
...
"#;
        ...
        assert!(
            template.is_ok(),
            "Failed to parse module without slot metadata"
        );
...
"#;
        ...
        assert!(power_core.required);
        assert!(power_core.has_varients);
...
"#;
    ...
    fn test_get_module_slot() {
        ...
        config
            .module_slots
            .insert("power-core".to_string(), power_core_slot);

        // Test successful lookup
        assert!(config.get_module_slot("power-core").is_some());
        assert_eq!(
            config.get_module_slot("power-core").unwrap().name,
            "Power Core"
        );
...
"#;
        ...
        assert_eq!(
            variants.len(),
            2,
            "Expected 2 variant types, got {}",
            variants.len()
        );
        assert!(
            variants.contains_key("power-core"),
            "Missing power-core variants"
        );
        assert!(
            variants.contains_key("kinetic-weapon"),
            "Missing kinetic-weapon variants"
        );
...
"#;
        ...
        assert!(
            result
                .unwrap_err()
                .contains("does not match any known module slot")
        );
