...
//!
//! The task is [`MgeTask`], fast and provisional: its manifest `manifests/mge.toml` records the
//! parameters above, which are constants of this module, and a manifest that disagrees with them
//! is refused.
...
/// How closely the expansions follow their profiles: the acceptance figures in the header.
#[derive(Debug, Clone, Copy, PartialEq)]
pub struct MgeResiduals {
    /// The largest relative error of [`MgeTables::exp`] against `e^(−s)` at 400 log-spaced `s`
    /// over 0.05–10, plan 15's P15.T3.a range.
    pub exp_worst_relative: f64,
    /// The largest error of [`MgeTables::bar`] against [`bar_profile`] at the fit's samples, as a
    /// fraction of the central value.
    pub bar_worst: f64,
    /// The relative error of the bar expansion's mass against [`bar_mass`].
    pub bar_mass_relative: f64,
}

/// The value at `x` of the expansion `table`.
fn expansion(table: &[(f64, f64); WIDTHS], x: f64) -> f64 {
    table
        .iter()
        .map(|&(w, sigma)| w * math::exp(-x * x / (2.0 * sigma * sigma)))
        .sum()
}

/// The acceptance figures of `tables`.
#[must_use]
pub fn residuals(tables: &MgeTables) -> MgeResiduals {
    let samples: [f64; 400] = log_spaced(0.05, 10.0);
    let exp_worst_relative = samples
        .iter()
        .map(|&s| (expansion(&tables.exp, s) / math::exp(-s) - 1.0).abs())
        .fold(0.0, f64::max);
    let step = BAR_REACH / index(BAR_SAMPLES - 1);
    let bar_worst = (0..BAR_SAMPLES)
        .map(|i| {
            let u = step * index(i);
            (expansion(&tables.bar, u) - bar_profile(u)).abs()
        })
        .fold(0.0, f64::max);
    let two_pi = 2.0 * core::f64::consts::PI;
    let mass: f64 = tables
        .bar
        .iter()
        .map(|&(w, sigma)| w * two_pi * sigma * sigma)
        .sum();
    MgeResiduals {
        exp_worst_relative,
        bar_worst,
        bar_mass_relative: mass / bar_mass() - 1.0,
    }
}
...
fn parameters() -> toml::Table {
    let text = format!(
        "widths = {WIDTHS}\nsweeps = {SWEEPS}\nexp_range = [{:?}, {:?}]\nexp_samples = \
         {EXP_SAMPLES}\nexp_widths = [{:?}, {:?}]\nbar_width = {BAR_WIDTH:?}\nbar_level = \
         {BAR_LEVEL:?}\nbar_end = {BAR_END:?}\nbar_reach = {BAR_REACH:?}\nbar_samples = \
         {BAR_SAMPLES}\nbar_mass_weight = {BAR_MASS_WEIGHT:?}\nbar_widths = [{:?}, {:?}]\n",
        EXP_RANGE.0, EXP_RANGE.1, EXP_WIDTHS.0, EXP_WIDTHS.1, BAR_WIDTHS.0, BAR_WIDTHS.1,
    );
...
#[must_use]
pub fn render(tables: &MgeTables) -> RustTable {
    let notes = format!(
        "\
...
/// The task: plan 02's first cut (P02.T6.a), fast, revision [`VERSION`].
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash, PartialOrd, Ord, Default)]
pub struct MgeTask;

impl FitTask for MgeTask {
    ...
    fn items(&self) -> &'static [&'static str] {
        &["MGE_EXP", "MGE_HOLED_EXP", "MGE_BAR"]
    }

    /// The thin disc's hole in scale lengths, the one number of the sim's galaxy model the fit
    /// reads.
    fn fingerprint(&self) -> SimFingerprint {
        SimFingerprint::new(vec![(
            "galaxy::fields::disc::THIN_DISC_HOLE_LENGTHS".to_owned(),
            THIN_DISC_HOLE_LENGTHS,
        )])
    }
    ...
    fn run(&self, manifest: &Manifest, _threads: NonZeroUsize) -> Result<TaskOutput, RunTaskError> {
        ...
        let tables = fit();
        let r = residuals(&tables);
        Ok(TaskOutput {
            table: render(&tables),
            source: "the profiles of plan 02's mass model (Design note 6), expanded as sums of \
                     Gaussians after Emsellem, Monnet and Bacon (1994, A&A 285, 723)"
                .to_owned(),
            acceptance: format!(
                "`MGE_EXP` within {:.3}% of e^(−s) over s = 0.05–10; `MGE_BAR` within {:.2}% of \
                 its central value over u = 0–{BAR_REACH} and its mass within {:.3}%",
                100.0 * r.exp_worst_relative,
                100.0 * r.bar_worst,
                100.0 * r.bar_mass_relative.abs(),
            ),
            provisional: Some("P02.T6.a"),
        })
    }
}
