...
//! acknowledge the site and cite the papers, which the fitted table's header does. The raw files
//! are therefore read from a directory outside the repository ([`DEFAULT_DATA_DIR`], or `--data`),
//! downloaded from [`DOWNLOAD_URL`] (the files [`file_name`] names), and the table's header records
//! a digest of them ([`WdCoolingTable::digest`]), so that a rerun can tell it has the same input.
...
/// Where `run wd_cooling` reads the sequences unless `--data` says otherwise: `target/data/
/// montreal_cooling/` under the repository's root, outside version control.
pub const DEFAULT_DATA_DIR: &str = concat!(
    env!("CARGO_MANIFEST_DIR"),
    "/../../target/data/montreal_cooling"
);
...
/// The committed source of `crates/hyperion-sim/src/tables/wd_cooling.rs`.
#[must_use]
pub fn render(table: &WdCoolingTable) -> String {
    let (lo, hi) = LOG_LUMINOSITY_RANGE;
    let mut out = format!(
        "\
...
//! interpolates under the generator's default recipe.
//!
//! Generated by `hyperion-fit run wd_cooling`, version {VERSION}. Do not edit: `cargo test -p
//! hyperion-fit` renders the fit again, where the sequences are present, and fails unless this
//! file matches it byte for byte.
//!
//! Source: the evolutionary sequences of Bédard, Bergeron, Brassard and Fontaine (2020, ApJ 901,
//! 93), thick hydrogen layers (`q_H` = 10⁻⁴, `q_He` = 10⁻²) on equimassic carbon–oxygen cores, the
//! {SEQUENCES} files `seq_020_thick.txt` to `seq_130_thick.txt` of
//! <https://www.astro.umontreal.ca/~bergeron/CoolingModels/>, retrieved 2026-09-24 (FNV-1a digest
//! {digest:#018x}); with thanks to the Montreal group. The sequences are not redistributed here.
//! Luminosities are converted with the IAU 2015 nominal solar luminosity.
//!
