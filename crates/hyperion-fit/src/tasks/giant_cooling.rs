...
#[must_use]
pub fn render(table: &GiantCoolingTable) -> RustTable {
    let r = &table.residuals;
    let notes = format!(
        "\
The grid is committed, with its provenance, as
`{GRID_FILE}`;
its masses, radii and luminosities are converted to the IAU 2015 nominal units.
...
largest at {r_mass} Jupiter masses and {r_age} Gyr.",
    ...
    RustTable {
        summary: vec![
            format!(
                "The cooling of giant planets from {} to {} Jupiter masses, as a table in log mass and log",
                MASS_RANGE_MJUP.0, MASS_RANGE_MJUP.1
            ),
            "age (plan 13, P13.T5.b), which `stellar::substellar::giant_cooling` interpolates."
                .to_owned(),
        ],
...
fn parameters() -> toml::Table {
    let text = format!(
        "masses = {MASSES}\nages = {AGES}\nmass_range_mjup = [{:?}, {:?}]\nage_range_yr = [{:?}, \
         {:?}]\nsmoothing = {SMOOTHING:?}\n",
        MASS_RANGE_MJUP.0, MASS_RANGE_MJUP.1, AGE_RANGE_YR.0, AGE_RANGE_YR.1,
    );
    toml::from_str(&text).expect("the parameters' own text is TOML")
}

/// The dataset the task reads: Sonora Bobcat's rows, committed.
pub const DATASET: &str = "giant_cooling";

/// The task: plan 13's P13.T5.b, fast, revision [`VERSION`].
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash, PartialOrd, Ord, Default)]
pub struct GiantCoolingTask;

impl FitTask for GiantCoolingTask {
    ...
    /// The sim's units the grid is converted into: the Jupiter mass and the nominal solar radius
    /// and luminosity.
    fn fingerprint(&self) -> SimFingerprint {
        SimFingerprint::new(vec![
            ("units::consts::JUPITER_MASS_KG".to_owned(), JUPITER_MASS_KG),
            (
    ...
    fn run(&self, manifest: &Manifest, _threads: NonZeroUsize) -> Result<TaskOutput, RunTaskError> {
        ...
        // The fit parses the grid compiled into this crate; loading the dataset checks that the
        // committed file is the one its provenance records.
        let dataset = manifest.load_dataset(DATASET)?;
        if dataset
            .files
            .iter()
            .any(|(_, bytes)| bytes.as_slice() != GRID.as_bytes())
        {
            return Err(RunTaskError::Input {
                task: "giant_cooling",
                source: "the grid on disk is not the one compiled in: rebuild".into(),
            });
        }
        let table = fit();
        let r = &table.residuals;
        Ok(TaskOutput {
            source: format!(
                "the Sonora Bobcat evolution tables (Marley et al. 2021, ApJ 920, 85), cloudless, \
                 at \\[M/H\\] = 0 and solar C/O, from Zenodo record 5063476 \
                 (doi:10.5281/zenodo.5063476, licence CC BY 4.0), retrieved 2026-09-23: their {} \
                 rows from 0.0005 to 0.011 M☉",
                r.rows
            ),
            acceptance: format!(
                "over the {} rows, fit minus grid, log₁₀ L within {} dex (rms {}) and R within \
                 {}% (rms {}%)",
                r.rows,
                rounded(r.worst_log_luminosity.residual.abs(), 3),
                rounded(r.rms_log_luminosity, 3),
                rounded(100.0 * r.worst_radius.residual.abs(), 2),
                rounded(100.0 * r.rms_radius, 2),
            ),
            table: render(&table),
            provisional: None,
        })
