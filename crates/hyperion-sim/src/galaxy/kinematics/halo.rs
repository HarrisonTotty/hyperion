...
//!
//! `σ_r²(r) = (1 ÷ (ν r^2β)) ∫_r^∞ ν r′^2β v_c²(r′) ÷ r′ dr′`, with `ν` the component's density
//! along its major axis, without the sphere that cuts it for placement, and `v_c` the in-plane
//! circular speed taken as spherical, tabulated on 64 radii. For a power law of slope γ in a flat
//! curve this is the brainstorm's `v_c² ÷ (γ − 2β)`, and unlike that form it stays finite in the
//! core. `σ_θ² = σ_φ² = (1 − β) σ_r²`.
//!
...
//! the in-situ component 0.3 and prograde at 0.35 `v_c` (provisional: the Splash rotates at about
//! 25 km/s, 0.1 `v_c`, Belokurov et al. 2020, MNRAS 494, 3880); the globular-born debris 0.5 and
//! none; each lesser progenitor β uniform on 0.3–0.7 and a rotation uniform on ±0.25 `v_c`, drawn
//! on `halo.kinematics` keyed by its item number. The mixture is tested against an anisotropy near
//! 0.6 and a radial dispersion near 141 km/s (Bond et al. 2010, ApJ 716, 1: (141, 75, 85) ± 5 km/s,
//! β = 0.68).
//!
...
/// β and net rotation of the in-situ component (plan 08, Design note 9; provisional, see the
/// module documentation).
pub const IN_SITU: (f64, f64) = (0.3, 0.35);
...
impl HaloComponentKinematics {
    ...
    pub fn at(&self, r: f64, sin_theta: f64, v_c: f64) -> ([f64; 3], [f64; 3]) {
        ...
        let sigma_t = sigma_r * (1.0 - self.beta).sqrt();
        (
...
/// let halo = HaloKinematics::new(Seed::new(1), &params, &tables);
/// let dominant = halo
///     .components()
///     .iter()
...
impl HaloKinematics {
    /// The laws of the halo of `params` in the in-plane curve of `potential`; the lesser
    /// progenitors' draws are keyed by `seed`.
    #[must_use]
    pub fn new(seed: Seed, params: &GalaxyParams, potential: &PotentialTables) -> Self {
        ...
        let components = params
            ...
            .map(|c| {
                ...
                HaloComponentKinematics {
                    ...
                    beta,
                    rotation,
                    sigma_r_sq: radial_dispersion_sq(|r| ln_density(c, r), beta, v_c_sq),
                }
            })
            .collect();
        Self {
...
/// `σ_r²` at the table's radii for the tracer `ln ν(r)`, anisotropy `beta` and curve `v_c²(r)`:
/// the integral in `ln r` of `ν r^2β v_c²`, by an 8-point rule between table radii and on
/// doubling panels beyond, summed downwards with the rescaling of the discs' integral.
pub(crate) fn radial_dispersion_sq(
    ln_nu: impl Fn(f64) -> f64,
    beta: f64,
    v_c_sq: impl Fn(f64) -> f64,
) -> [f64; HALO_RADII] {
    let weight = |r: f64| ln_nu(r) + 2.0 * beta * math::ln(r);
...
mod tests {
    ...
    fn a_power_law_in_a_flat_curve_gives_the_closed_form() {
        ...
        for beta in [0.0, 0.3, 0.6, 0.9] {
            let table = radial_dispersion_sq(|r| -3.5 * math::ln(r), beta, |_| v_c * v_c);
