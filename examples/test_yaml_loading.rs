...
use hyperion::config::{AmmunitionConfig, ModuleConfig, ShipClassConfig, WeaponConfig};
use std::fs;
...
fn main() {
    ...
    match fs::read_to_string("data/ship-classes/cruiser.yaml") {
        Ok(yaml) => match serde_yaml::from_str::<ShipClassConfig>(&yaml) {
            Ok(mut config) => {
                config.set_id("cruiser".to_string());
                println!("✓ Loaded ship class: {}", config.name);
                println!("  Size: {:?}, Role: {:?}", config.size, config.role);
                println!(
                    "  Base hull: {}, Max modules: {}",
                    config.base_hull, config.max_modules
                );
    ...
    match fs::read_to_string("data/modules/power-cores/mark-iii-fission-reactor.yaml") {
        Ok(yaml) => match serde_yaml::from_str::<ModuleConfig>(&yaml) {
            Ok(mut config) => {
                ...
                println!(
                    ...
                    config.max_energy, config.production
                );
        ...
        Ok(yaml) => match serde_yaml::from_str::<ModuleConfig>(&yaml) {
            Ok(mut config) => {
                ...
                println!(
                    ...
                    config.thrust, config.energy_consumption
                );
                println!("  Weight: {}, Cost: {}", config.weight, config.cost);
            }
    ...
    match fs::read_to_string("data/modules/kinetic-weapons/100mm-medium-cannon.yaml") {
        Ok(yaml) => match serde_yaml::from_str::<WeaponConfig>(&yaml) {
            Ok(mut config) => {
                config.set_id("100mm-medium-cannon".to_string());
                println!("✓ Loaded weapon: {}", config.name);
                println!("  Model: {}, Kind: {}", config.model, config.kind);
                println!(
                    "  Reload time: {}, Accuracy: {}",
                    config.reload_time, config.accuracy
                );
                println!("  Weight: {}, Cost: {}", config.weight, config.cost);
            }
    ...
    match fs::read_to_string("data/modules/kinetic-weapons/ammo/he-100mm-shell.yaml") {
        Ok(yaml) => match serde_yaml::from_str::<AmmunitionConfig>(&yaml) {
            Ok(mut config) => {
                config.set_id("he-100mm-shell".to_string());
                println!("✓ Loaded ammunition: {}", config.name);
                println!("  Type: {}, Size: {}", config.ammo_type, config.ammo_size);
                println!(
                    "  Impact damage: {}, Velocity: {}",
                    config.impact_damage, config.velocity
                );
