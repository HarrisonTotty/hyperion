...
describe("RequestStatus", () => {
  ...
  it("announces its own changes, being a region of its own", () => {
    render(<RequestStatus state={{ kind: "pending" }} />);

    // An `output` is a live region by itself, so it needs no `aria-live` to be read. It is never
    // rendered inside another region: a caller whose region must carry the state renders the words
    // from `annunciation` instead (the orchestrator's ruling 13).
